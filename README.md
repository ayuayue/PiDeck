...
### v0.6.4 更新
- 🚀 **计划模式**：Composer 新增计划模式，Agent 先生成计划再逐条确认执行
- 🚀 **ask_question 扩展增强**：批量提问、结构化选项、结果持久化、卡片优化
- 🚀 **消息编辑/删除**：AI 回答和用户消息支持复制、编辑、删除
- 🚀 **草稿本浮层**：全新的草稿本 ScratchPad，支持预览勾选和动画
- 🚀 **pi-deck-todo 内置扩展**：新增任务管理扩展
- 🚀 **内容行宽限制**：可拖拽的内容宽度滑块，默认不限
- 🚀 **Thinking 块重构**：ThinkingBlock 卡片化、原位交替、默认展开、状态指示器
- 🚀 **扩展管理增强**：禁用/启用内置扩展，项目级技能管理
- 🚀 **信任确认系统**：桌面端拦截信任确认，不信任也可打开
- ✨ 思考卡片呼吸动画、Web 搜索卡片二级标题、Composer 高度优化
- ✨ 终端/抽屉平滑动画、聊天区背景统一、工具卡片修复
- 🐛 暗色模式白色背景修复、RichInput 换行修复、plan 模式死锁修复
- 🐛 Linux Wayland 修复、飞书修复、消息渲染修复
...
本应用启动本地 `pi` 进程并通过 Electron IPC 暴露有限的文件操作。请仅运行你信任的源码。本应用不会采集任何用户数据或发送使用统计。pi agent 子进程代理和桌面端模型拉取/测试代理可独立配置；系统浏览器打开的外部链接仍由系统浏览器网络设置决定。
