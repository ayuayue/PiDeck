...
- **Grouped process timeline (opt-in)** — Consecutive thinking and tool calls inside one turn fold into a process group: the header shows an activity-category summary, the body is height-capped and scrolls on its own, while intermediate replies, retries and errors stay first-class rows and the final answer never folds away. The new *Process group display* setting controls it and is off by default, so the flat layout stays available with one click. Group headers are sized and weighted below their content, sit on the conversation text track, and no longer steal wheel or bottom-following gestures from the timeline.
...
- **Commit & push quick actions** — When the latest turn reads like finished work, a suggestion strip appears above the composer; clicking it sends the matching "commit" / "push" prompt directly, without going through the draft, and the suggestions recompute on every new turn.
...
### 🙏 Thanks
- Thanks to **@TheMapleBin** for the path-linkification text-loss fix (#274 / #276), **@525300887039** for reporting the Chinese-slash misdetection (#277), **@cmyk-xing** for the looser provider names, the DSH add-provider form, the narrow-window layout and the simple navigation mode (#268), and **@juzijun233** for the model configuration export/import panels (#261).
