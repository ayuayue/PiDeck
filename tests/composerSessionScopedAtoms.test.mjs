...
const composerAtoms = loadTsCommonJs(
  "src/renderer/src/atoms/composer-atoms.ts",
  {
    stubs: {
      "./session-atoms": {
        currentSessionIdAtom: { debugLabel: "currentSessionIdAtom" },
      },
    },
  },
);
...
test("sessionDraftBySessionIdAtomFamily：写别的会话不通知本会话订阅者", () => {
  ...
  store.set(composerAtoms.setSessionDraftAtom, {
    sessionId: "session-b",
    value: "hello",
  });
...
test("attachments/pasteFiles 默认值稳定：未写过的会话跨写不重发通知", () => {
  const store = createStore();
  const attachmentsA =
    composerAtoms.sessionAttachmentsBySessionIdAtomFamily("session-a");
  let attachmentsNotify = 0;
  store.sub(attachmentsA, () => {
    attachmentsNotify += 1;
  });
  ...
  store.set(composerAtoms.setSessionAttachmentsAtom, {
    sessionId: "session-b",
    value: [{}],
  });
  ...
  const pasteFilesB =
    composerAtoms.sessionPasteFilesBySessionIdAtomFamily("session-b");
...
test("sendState/mode/quotes family：默认值与缺省读取", () => {
  const store = createStore();
  const sendStateA =
    composerAtoms.sessionSendStateBySessionIdAtomFamily("session-a");
  assert.deepEqual(JSON.parse(JSON.stringify(store.get(sendStateA))), {
    status: "idle",
  });

  const modeA =
    composerAtoms.sessionComposerModeBySessionIdAtomFamily("session-a");
  ...
  store.set(composerAtoms.setSessionQuotesAtom, {
    sessionId: "session-b",
    value: {},
  });
...
test("idleSessionSendState 常量导出且跨读相等（供 runtime 控制器复用）", () => {
  assert.equal(
    composerAtoms.idleSessionSendState,
    composerAtoms.idleSessionSendState,
  );
  assert.deepEqual(
    JSON.parse(JSON.stringify(composerAtoms.idleSessionSendState)),
    { status: "idle" },
  );
});
