...
function tab(id, filePath, tabKey, projectId) {
	const value = tabKey === undefined ? { id, filePath } : { id, filePath, tabKey };
	return projectId === undefined ? value : { ...value, fileAccessScope: { projectId } };
}

test("openPreviewEditorTab: 单击替换预览，不降级常驻 Tab", () => {
	const { openPreviewEditorTab } = loadEditorTabs();
	const first = openPreviewEditorTab([tab("a", "/a.ts")], null, tab("b", "/b.ts"));
	assert.equal(json(first.tabs.map((t) => t.id)), json(["a", "b"]));
	assert.equal(first.previewId, "b");
	assert.equal(first.activeId, "b");

	const replaced = openPreviewEditorTab(first.tabs, first.previewId, tab("c", "/c.ts"));
	assert.equal(json(replaced.tabs.map((t) => t.id)), json(["a", "c"]));
	assert.equal(replaced.previewId, "c");

	const resident = openPreviewEditorTab([tab("a", "/a.ts"), tab("c", "/c.ts")], "c", tab("new-a", "/a.ts"));
...
test("openPermanentEditorTab: 双击升格预览为常驻", () => {
	...
	const next = openPermanentEditorTab([tab("a", "/a.ts"), tab("b", "/b.ts")], "b", tab("b", "/b.ts"));
...
test("openPermanentEditorTab: 新文件追加且保留其它预览", () => {
	const { openPermanentEditorTab } = loadEditorTabs();
	const next = openPermanentEditorTab([tab("a", "/a.ts"), tab("p", "/p.ts")], "p", tab("b", "/b.ts"));
...
test("openPreviewEditorTab: 同 path+tabKey 才视为同一文件", () => {
	const { openPreviewEditorTab } = loadEditorTabs();
	const next = openPreviewEditorTab([tab("a", "/a.ts", "k1")], null, tab("b", "/a.ts", "k2"));
	assert.equal(json(next.tabs.map((t) => t.id)), json(["a", "b"]));
...
test("editor tab identity keeps project-scoped buffers separate", () => {
	const { openPermanentEditorTab } = loadEditorTabs();
	const differentProject = openPermanentEditorTab([tab("a", "/shared/file.ts", undefined, "project-a")], null, tab("b", "/shared/file.ts", undefined, "project-b"));
	assert.equal(json(differentProject.tabs.map((t) => t.id)), json(["a", "b"]), "the same host path under another project authorization must get its own buffer");
