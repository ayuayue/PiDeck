...
test("solo ChatSessionPane is reused across tab switches", () => {
	...
	const soloBlock = appSource.slice(appSource.indexOf("solo={"), appSource.indexOf("soloSessionId="));
...
test("file tree list is shallow by default in the drawer and accepts a scoped directory", () => {
	...
	// 链式调用与括号表达式可能被格式化调整（desktopApi.files 与 .list 分行、外层补括号）：
	// 只锁「按 effectiveProjectId + maxDepth 0 拉一层」的语义。
	assert.match(composerSource, /desktopApi\s*\.?\s*files\s*\.?\s*list\(\s*effectiveProjectId,\s*\{\s*maxDepth: 0\s*\}\)/);
