...
it("迁移进行中 append 的新行不得被迁移快照覆盖丢失（M6）", async () => {
	...
	const { ImageSessionStore } = loadTsCommonJs(
	...
	const appending = store.append(SESSION_ID, [
		{
			id: "m-new",
			role: "user",
			text: "appended-during-migration",
			timestamp: 2,
		},
	]);
	await waitFor(
		() => readFileSync(file, "utf8").includes("appended-during-migration"),
		300,
	);
	...
	assert.ok(
		raw.includes("appended-during-migration"),
		"迁移窗口内 append 的行必须存活",
	);
	assert.match(
		raw,
		/"ref":"a{64}\.png"/,
		"迁移本身必须已完成（旧行已改写为引用格式）",
	);
});
...
it("并发 append 之间也必须串行（M6 附属回归）", async () => {
	...
	for (let i = 0; i < 20; i += 1) {
		writes.push(
			store.append(sid, [
				{ id: `p-${i}`, role: "user", text: `t-${i}`, timestamp: i + 1 },
			]),
		);
	}
