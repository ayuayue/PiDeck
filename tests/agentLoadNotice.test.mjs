...
function loadModule() {
	...
	vm.runInNewContext(
		transpile("src/renderer/src/utils/agentLoadNotice.ts"),
		sandbox,
		{
...
test("useAgentLoadNotice warns once per launch via guarded effect", () => {
	const source = readFileSync(
		"src/renderer/src/hooks/useAgentLoadNotice.ts",
		"utf8",
	);
	...
	assert.match(
		source,
		/countActivatedAgents\(store\.get\(sessionRuntimeByIdAtom\)\)/,
	);
...
test("i18n copy covers zh-CN and en-US with count placeholder", () => {
	...
	assert.match(
		zh,
		/"app\.highAgentCountTitle":\s*"运行的 Agent 有点多，给电脑减减负吧"/,
	);
	assert.match(zh, /"app\.highAgentCountBody":\s*"当前有 \{count\} 个 Agent/);
	assert.match(
		en,
		/"app\.highAgentCountTitle":\s*"Quite a few agents are running/,
	);
	assert.match(
		en,
		/"app\.highAgentCountBody":\s*['"]There are currently \{count\} active agents/,
	);
	// 静默按钮双语同步
