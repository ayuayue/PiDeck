...
test("stop() 后迟到的 message_start 不得为死 agentId 重建消息（M2 回归）", () => {
	...
	assert.equal(
		manager.messages.has("agent-dead"),
		false,
		"不得为死 agentId 重建 messages 键",
	);
	assert.equal(
		manager.streamingAgents.has("agent-dead"),
		false,
		"不得重激活流式状态",
	);
});
...
test("源码契约：六个流式分支的闸门都必须同时拒绝无 runtime", () => {
	...
	const guarded =
		src.match(/if \(!runtime \|\| this\.isAgentStreamSealed\(agentId\)\)/g) ?? [];
	assert.equal(
		guarded.length,
		6,
		"message_start/update/end + tool_execution_start/end/update",
	);
	// 最小改动版：仅 :5670（handleAssistantMessageEvent 内、message_update 分支被拦后
	// 不可达的纵深守卫）允许残留一个只查封印的旧写法
	const bareSeal =
		src.match(/if \(this\.isAgentStreamSealed\(agentId\)\)/g) ?? [];
