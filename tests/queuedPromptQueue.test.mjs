...
const sessionRuntimeInjectorSource = readFileSync("src/renderer/src/components/session/SessionRuntimeInjector.tsx", "utf8");
const bootstrapSource = readFileSync("src/renderer/src/components/app/AppBootstrap.tsx", "utf8");
const globalListenersSource = readFileSync("src/renderer/src/hooks/useGlobalAgentListeners.ts", "utf8");
const composerPanelsSource = readFileSync("src/renderer/src/components/session/ComposerPanels.tsx", "utf8");
const stylesSource = readRendererStyles();
const i18nSource = [readFileSync("src/renderer/src/i18n/rendererCopy.zh-CN.ts", "utf8"), readFileSync("src/renderer/src/i18n/rendererCopy.en-US.ts", "utf8")].join("\n");
const runtimeStateSource = readFileSync("src/renderer/src/utils/agentRuntimeState.ts", "utf8");
const queueStateSource = readFileSync("src/renderer/src/utils/queuedPromptQueue.ts", "utf8");
const toolRuntimeStateSource = readFileSync("src/shared/toolRuntimeState.ts", "utf8");
...
const sharedTypesSource = [readFileSync("src/shared/types.ts", "utf8"), readFileSync("src/shared/types/session.ts", "utf8")].join("\n");
// Queue ownership now lives in useQueuedPrompt.
const queuedPromptHookSource = readFileSync("src/renderer/src/hooks/useQueuedPrompt.ts", "utf8");
const composerControllerSource = readFileSync("src/renderer/src/hooks/useSessionComposerController.ts", "utf8");
const sessionSendSource = readFileSync("src/renderer/src/hooks/useSessionSend.ts", "utf8");
const sessionRuntimeBridgeSource = readFileSync("src/renderer/src/hooks/useSessionRuntimeBridge.ts", "utf8");
const sessionRuntimeControllerSource = readFileSync("src/renderer/src/hooks/useSessionRuntimeController.ts", "utf8");
...
test("pending prompts render inside the composer before composer-box", () => {
	...
	assert.ok(queuePanelIndex >= 0, "QueuedPromptPanel should exist in SessionRuntimeInjector");
...
test("indeterminate prompt timeout never becomes a retryable rejection", () => {
	assert.match(sharedTypesSource, /delivery: "unknown"/);
	assert.match(agentManagerSource, /catch \(error\)[\s\S]*?delivery: "unknown"/);
	assert.match(agentManagerSource, /命令接收结果未知[\s\S]*?delivery: "unknown"/);
	...
	const runtimeControllerSource = readFileSync("src/renderer/src/hooks/useSessionRuntimeController.ts", "utf8");
...
test("prompt acceptance is explicit across the main and renderer boundary", () => {
	...
	assert.match(agentManagerSource, /accepted: false,[\s\S]{0,160}?error: errorMessage,[\s\S]{0,160}?i18nKey: "diagnostic\./);
