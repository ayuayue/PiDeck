...
const panelControls = readFileSync("src/renderer/src/components/app/git/GitPanelControls.tsx", "utf8");
...
describe("Git panel VS Code Source Control contract", () => {
	...
	test("keeps mutation locked until IPC settles and times out the real git commands", () => {
		...
		assert.ok((gitService.match(/timeout(?:Ms)?: GIT_MUTATION_TIMEOUT_MS/g) ?? []).length >= 7, "all mutation and mutation-validation git commands should have a process timeout");
	});
	...
	test("hosts Git Diff in the middle workbench with split/maximize toggle", () => {
		...
		const workbenchContent = readFileSync("src/renderer/src/components/workspace/WorkbenchContent.tsx", "utf8");
	...
	test("keeps only the newest Git diff request and invalidates pending work on every close", () => {
		...
		// 依赖数组可能被格式化换行（[workspace, gitDrawerDiff] 跨行）：容忍内部空白。
		const toolDrawerAction = app.match(/handleToolDrawerAction = useCallback[\s\S]*?\},?\s*\[\s*workspace,\s*gitDrawerDiff/)?.[0] ?? "";
		assert.match(toolDrawerAction, /if \(panel === "git" && gitDrawerDiff\) \{\s*closeGitDiff\(\);\s*return;\s*\}/);
		// git 入口收进抽屉活动栏（rail，与 outline 一致走共享 handler，受同一开关门控）
		// 三元与数组元素可能被格式化换行：在条件、id 与 onClick 之间都容忍空白。
		assert.match(app, /\.\.\.\(settings\.enableGitManagement && activeProjectId\s*\?[\s\S]*?id: "git"[\s\S]*?onClick: \(\) => handleToolDrawerAction\("git"\)/);
	});
