...
describe("Git panel VS Code Source Control contract", () => {
  test("uses a CSS triangle twistie without structural icon imports", () => {
    const twistie = sourceBetween(
      "function Twistie(",
      "function GitStageGlyph(",
    );
    assert.match(twistie, /before:content-\['▶'\]/);
    assert.doesNotMatch(twistie, /\.git-twistie/);
    assert.doesNotMatch(
      twistie,
      /ChevronDown|ChevronRight|GitBranch|GitCommit|GitCompare|GitGraph|Ellipsis|Minus|Plus/,
    );
  });
  ...
  test("uses exactly three independently collapsible persisted panes with Changes open by default", () => {
    ...
    assert.match(
      panel,
      /pideck:git-panel:\$\{projectId\}:\$\{encodeURIComponent\(repoScopeKey\)\}:pane-state:\$\{suffix\}/,
    );
  ...
  test("provides visible-adjacent pointer and keyboard-accessible resize sashes", () => {
    ...
    assert.match(
      panel,
      /adjacentVisiblePane\(\s*visibleOpen,\s*"changes",\s*1,\s*\)/,
    );
    assert.match(
      panel,
      /renderSash\(\s*"changes",\s*visibleSashAfterChanges\s*\)/,
    );
  ...
  test("keeps resource groups inside Changes and retains VS Code decorations", () => {
    ...
    assert.match(
      panel,
      /groups\.merge\.length \+ stagedCount \+ workingChanges\.length/,
    );
  ...
  test("renders SVG graph lanes and does not retain the old fixed graph height", () => {
    ...
    assert.match(
      graph,
      /lastNodeIndex\(\s*output,\s*commit\.parents\[parentIndex\],\s*\)/,
    );
    ...
    assert.doesNotMatch(
      styles,
      /\.git-history-list\s*\{[^}]*max-height:\s*310px/,
    );
  });
  ...
  test("prefers Electron system language data while preserving explicit locale choices", () => {
    ...
    assert.match(
      i18n,
      /mode === "zh-CN" \|\| mode === "en-US" \|\| mode === "pseudo"/,
    );
    assert.match(
      i18n,
  ...
  test("guards async state and constrains visible pane heights", () => {
    ...
    assert.match(
      panel,
      /request === statusRequestRef\.current\s*&&\s*projectId === projectIdRef\.current/,
    );
    ...
    assert.match(
      panel,
      /Math\.min\(\s*requestedBefore,\s*startBeforeHeight \+ startAfterHeight - PANE_MIN_BODY_HEIGHT,\s*\)/,
    );
    assert.match(panel, /flushPendingHeights\(\)/);
    assert.match(
      panel,
      /const hasChangesToCommit\s*=\s*stagedCount > 0\s*\|\|\s*\(workingChanges\.length > 0/,
    );
    assert.match(panel, /if \(stagedCount > 0\)[\s\S]*?runCommit\(false\)/);
    assert.match(
      panel,
      /smartCommitPreference\.enableSmartCommit[\s\S]*?runCommit\(true\)/,
    );
    ...
    assert.match(
      panel,
      /await props\.stageFiles\(projectId, paths\)[\s\S]*?await props\.commit\(projectId, message\)/,
    );
    ...
    assert.doesNotMatch(
      panelControls,
      /className="fixed min-w-0 max-w-\[calc\(100vw-16px\)\]/,
    );
    assert.doesNotMatch(
      panelControls,
    ...
    assert.match(
      panel,
      /const preferredWidth = Math\.max\(Math\.ceil\(rect\.width\),\s*160\)/,
    );
    assert.match(
      panel,
      /\{\s*preferredWidth,\s*maxHeight:\s*300,\s*gap:\s*2\s*\}/,
    );
    ...
    assert.doesNotMatch(
      panel,
      /min-w-48 overflow-auto rounded-md border border-border bg-popover/,
    );
    ...
    assert.doesNotMatch(
      panelControls,
      /min-w-0 flex-1 truncate font-mono text-\[13px\] font-semibold/,
    );
    ...
    assert.match(
      graph,
      /font-mono text-xs text-\[var\(--git-desc-fg\)\]">\{commit\.hash\}/,
    );
    assert.match(styles, /--git-ui-font:\s*var\(--font-family-base\)/);
    assert.match(
      cssRule("\\.git-commit-hover-avatar"),
      /font-family:\s*var\(--git-ui-font\)/,
    );
  });
  ...
  test("runs silent refreshes without overlapping slow status requests", () => {
    assert.match(
      panel,
      /const statusRunningRequestRef = useRef<\{\s*projectId: string;\s*repoScopeKey: string;\s*request: number;\s*\} \| null>\(null\)/,
    );
    assert.match(
      panel,
  ...
  test("keeps mutation locked until IPC settles and times out the real git commands", () => {
    ...
    assert.doesNotMatch(
      panel,
      /setTimeout\([\s\S]*?mutationRunningRef\.current = false/,
    );
    assert.match(gitService, /const GIT_MUTATION_TIMEOUT_MS = 30_000;/);
    assert.ok(
      (gitService.match(/timeout(?:Ms)?: GIT_MUTATION_TIMEOUT_MS/g) ?? [])
        .length >= 7,
  ...
  test("shows details only after a short mouse hover and lazily expands files on click", () => {
    ...
    assert.match(
      graph,
      /window\.setTimeout\([\s\S]*?COMMIT_HOVER_OPEN_DELAY_MS/,
    );
    assert.match(graph, /const COMMIT_HOVER_DISMISS_DELAY_MS = 400/);
    assert.match(
      graph,
      /window\.setTimeout\([\s\S]*?COMMIT_HOVER_DISMISS_DELAY_MS/,
    );
    assert.match(
      graph,
      /onClick=\{\(\) => \{[\s\S]*?dismissHover\(\);[\s\S]*?toggleCommit\(commit\.hash\);/,
    );
  ...
  test("opens committed files as isolated read-only first-parent diffs", () => {
    ...
    assert.match(
      app,
      /label: `\$\{diff\.path\.split[\s\S]*?\$\{commit\.shortHash\}/,
    );
  ...
  test("opens workspace resources into the middle workbench without drawer overlays", () => {
    ...
    assert.match(
      resourceTree,
      /groupType === "workingTree" \|\| props\.groupType === "untracked"/,
    );
    assert.match(resourceTree, /kind: "stage"/);
    assert.match(app, /api\.git\.workspaceFileDiff/);
    assert.match(
      app,
      /setGitDrawerDiff\(\{[\s\S]*?projectId,[\s\S]*?filePath: diff\.path/,
    );
  ...
  test("hosts Git Diff in the middle workbench with split/maximize toggle", () => {
    assert.match(
      styles,
      /\.file-diff-viewer\s*\{[\s\S]*?flex:\s*1 1 auto;[\s\S]*?width:\s*100%/,
    );
  ...
  test("keeps only the newest Git diff request and invalidates pending work on every close", () => {
    ...
    assert.match(
      app,
      /const request = \+\+gitDiffRequestSequenceRef\.current/g,
    );
    ...
    assert.match(
      app,
      /const dismissGitDiffOnly = useCallback\(\(\) => \{[\s\S]*?gitDiffRequestSequenceRef\.current \+= 1;[\s\S]*?setGitDrawerDiff\(null\)/,
    );
    // Git 面板再次点击时优先关闭 diff 详情：该语义现收拢在共享的
    // handleToolDrawerAction 中（outline 与抽屉活动栏共用，见 workspaceDrawer 测试）。
    const toolDrawerAction =
      app.match(
        /handleToolDrawerAction = useCallback[\s\S]*?\},\s*\[[\s\S]*?workspace,[\s\S]*?gitDrawerDiff/,
      )?.[0] ?? "";
    assert.match(
      toolDrawerAction,
      /if \(panel === "git" && gitDrawerDiff\) \{\s*closeGitDiff\(\);\s*return;/,
    );
    // git 入口收进抽屉活动栏（rail，与 outline 一致走共享 handler，受同一开关门控）
    assert.match(
      app,
      /\.\.\.\(settings\.enableGitManagement && activeProjectId\s*\?\s*\[\s*\{[\s\S]*?id: "git"[\s\S]*?onClick: \(\) => handleToolDrawerAction\("git"\)/,
    );
  });
  ...
  test("discard flows through literal-pathspec restore and trash for untracked files", () => {
    assert.match(
      preload,
      /discard: \(\s*projectId: string,\s*group: "workingTree" \| "untracked",\s*filePath: string,\s*repoPath\?: string,?\s*\)/,
    );
  ...
  test("provides a right-click paste menu on the commit input", () => {
    ...
    assert.match(
      preload,
      /clipboard: \{[\s\S]*?readText: \(\) => clipboardSync\(ipcChannels\.clipboardReadText, ""\)/,
    );
    assert.match(
      preload,
  ...
  test("bounds Git diff memory and validates renderer-controlled Git inputs", () => {
    ...
    assert.match(gitService, /Math\.min\(\s*500,/);
  ...
  test("loads commit files against the first parent and preserves rename origins", () => {
    ...
    assert.match(
      gitService,
      /\[\s*"diff",\s*"--name-status",\s*"-z",\s*"--find-renames",\s*commit\.parents\[0\],\s*commit\.hash,\s*\]/,
    );
    assert.match(
      gitService,
      /\[\s*"diff-tree",\s*"--root",\s*"--no-commit-id",\s*"--name-status",\s*"-r",\s*"-z",\s*"--find-renames",\s*commit\.hash,\s*\]/,
    );
