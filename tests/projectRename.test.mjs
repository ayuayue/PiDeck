...
const { sanitizeProjectDisplayName, PROJECT_DISPLAY_NAME_MAX_LENGTH } = loadTsCommonJs("src/main/projects/projectPathPolicy.ts");
...
test("sanitizeProjectDisplayName 超长拒绝", () => {
	assert.throws(() => sanitizeProjectDisplayName("x".repeat(PROJECT_DISPLAY_NAME_MAX_LENGTH + 1)), /PROJECT_NAME_TOO_LONG:/);
	assert.equal(sanitizeProjectDisplayName("x".repeat(PROJECT_DISPLAY_NAME_MAX_LENGTH)).length, PROJECT_DISPLAY_NAME_MAX_LENGTH);
});
...
test("rename 聊天项目拒绝", async () => {
	const { store, dir } = await makeProjectStore();
	try {
		await assert.rejects(() => store.rename("builtin-chat", "随便改"), /PROJECT_RENAME_NOT_ALLOWED/);
	} finally {
		rmSync(dir, { recursive: true, force: true });
	}
});
...
test("rename worktree 子项目拒绝（name 承载 git 分支名）", async () => {
	...
	try {
		...
		await assert.rejects(() => store.rename(child.id, "新名字"), /PROJECT_RENAME_NOT_ALLOWED/);
	} finally {
		rmSync(dir, { recursive: true, force: true });
	}
});
