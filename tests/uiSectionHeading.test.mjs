...
const heading = readFileSync("src/renderer/src/components/ui-shadcn/section-heading.tsx", "utf8");
const storage = readFileSync("src/renderer/src/components/app/settings/SettingsStorageTab.tsx", "utf8");
const piSettings = readFileSync("src/renderer/src/config/SettingsTab.tsx", "utf8");
const feedback = readFileSync("src/renderer/src/features/feedback/FeedbackDialog.tsx", "utf8");
...
test("settings and Pi management sections use the shared heading", () => {
	...
	// 断言「从共享 SettingRows 引入这三个行/盒原语」，不限制同时引入的其它原语
	// （例如 ClearableSettingsInput 也是同一模块的共享件）。
	assert.match(piSettings, /import \{[^}]*\bSettingBox\b[^}]*\bSettingRow\b[^}]*\bSettingSwitchRow\b[^}]*\} from "\.\.\/components\/app\/settings\/SettingRows"/);
