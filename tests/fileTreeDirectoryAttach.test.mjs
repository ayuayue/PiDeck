...
const menuSource = readFileSync("src/renderer/src/components/session/ComposerOverlayComponents.tsx", "utf8");
...
test("FileContextMenu：文件与目录都能「加入对话引用」和「默认方式打开」", () => {
	...
	assert.match(menuSource, /<DropdownMenuItem onSelect=\{props\.onAttach\}>\s*\{t\("menu\.attachFile"\)\}/);
	// 「默认方式打开」对目录同样开放：目录的默认处理器就是文件管理器。
	assert.match(menuSource, /<DropdownMenuItem onSelect=\{props\.onOpen\}>\s*\{t\("menu\.defaultOpen"\)\}/);
...
test("目录节点引用带尾斜杠（raw）且可解析为 file chip", () => {
	...
	const parsed = chips.parseRichInputChips(`看下 ${ref} 这个目录`, undefined, new Set([".tmp"]));
