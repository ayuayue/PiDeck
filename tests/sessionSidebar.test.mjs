...
function loadExpandedProjectsModule() {
	return loadTsModule("src/renderer/src/utils/sidebarExpandedProjects.ts", "sidebarExpandedProjects.ts", (specifier) => {
		throw new Error(`Unexpected import: ${specifier}`);
	});
}

function loadPillsModule() {
	return loadTsModule("src/renderer/src/sessionFilterPills.ts", "sessionFilterPills.ts", (specifier) => {
		throw new Error(`Unexpected import: ${specifier}`);
	});
}

function loadNavTabModule() {
	return loadTsModule("src/renderer/src/utils/sidebarNavTab.ts", "sidebarNavTab.ts", (specifier) => {
		throw new Error(`Unexpected import: ${specifier}`);
	});
}
...
test("source filters preserve all sources until the user narrows a project", () => {
	...
	assert.deepEqual(filterSidebarSessions(sessions, new Set(["codex"])), [{ source: "codex" }]);
	const saved = new Map();
...
function appFunctionBlock(source, name, nextName) {
	// 定位不能假设两空格缩进（formatter 统一为 tab）；块边界用行首缩进无关的锚点。
	const start = source.search(new RegExp(`^[\\t ]*async function ${name}\\(`, "m"));
	const end = source.search(new RegExp(`^[\\t ]*async function ${nextName}\\(`, "m"));
	...
	assert.ok(end > start, `${nextName} must come after ${name}`);
...
test("sidebar archive and delete dismiss nested subagent chats instead of leaving a composer", () => {
	...
	assert.match(chrome, /const closeTabs = useCallback\(\s*\(sessionIds: string\[\]\)/);
...
test("Chat section keeps an independent collapse control after the parent project row is hidden", () => {
	const projectTree = readFileSync("src/renderer/src/components/sidebar/ProjectTree.tsx", "utf8");
	const chatSection = projectTree.slice(projectTree.indexOf("const chatProjects"), projectTree.indexOf("    {workspaceProjects.length > 0"));
...
test("show-more row renders the count separately and drops the trailing unit", () => {
	...
	// formatter 会把单元素三元/逻辑渲染的包裹括号去掉：断言只锁 !props.nested 门控。
