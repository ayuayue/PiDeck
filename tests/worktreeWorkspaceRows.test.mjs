...
test("workspace title actions match the normal project toolbar", () => {
	const mainSection = worktreeTree.slice(worktreeTree.indexOf("workspace-tree-main"), worktreeTree.indexOf("workspace-tree-list"));
...
test("workspace title more-actions keeps safe worktree removal in the menu", () => {
	...
	// 属性值可能被格式化换行：用 \s* 容忍。
	assert.match(sidebarContent, /onRemoveWorktree=\{\s*menuProjectWorktreeParent/);
...
test("worktree removal keeps a fade-out animation keyed by project identity", () => {
	// The stable child project ID flows App → AppSidebar → SidebarContent → ProjectTree → WorktreeTree.
	assert.match(worktreeTree, /removingWorktreeProjectIds\?: ReadonlySet<string>/);
	assert.match(worktreeTree, /removing=\{removingProjectIds\.has\(row\.key\)\}/);
	assert.match(worktreeTree, /props\.removing && "worktree-removing"/);
	assert.match(projectTree, /removingWorktreeProjectIds\?: ReadonlySet<string>/);
	assert.match(projectTree, /removingWorktreeProjectIds=\{props\.removingWorktreeProjectIds\}/);
	assert.match(sidebarContent, /removingWorktreeProjectIds=\{props\.removingWorktreeProjectIds\}/);
	assert.match(sidebarComponents, /onRemoveWorktree \?\? props\.onRemoveProject/);
	assert.match(workspaceStyles + readFileSync("src/renderer/src/styles/foundation.css", "utf8"), /\.workspace-tree-row\.worktree-removing\s*\{[\s\S]*?opacity: 0;/);
});
