...
test("ThinkingUpdate payload carries stable id + lifecycle fields", () => {
 ...
 assert.doesNotMatch(
  agentTypes.slice(
   agentTypes.indexOf("export type ThinkingUpdate"),
   agentTypes.indexOf("export type ComposerAgentMode"),
  ),
...
test("main process: thinking_delta does not upsert; thinking_end does not write messages", () => {
 const thinkingDeltaIdx = agentManager.indexOf(
  'if (eventType === "thinking_delta")',
 );
 const thinkingEndIdx = agentManager.indexOf(
...
test("main process: finalize writes message.thinking before done clears live channel", () => {
 ...
 const finalizeThenFinish =
  /finalizeThinkingIntoMessage[\s\S]{0,200}?flushMessageEmit[\s\S]{0,80}?finishThinkingChannel/;
...
test("main process: upsertAssistantMessage no longer dual-writes thinking mid-stream", () => {
 ...
 const nextPrivate = agentManager.indexOf(
...
test("renderer: releasing live thinking also removes atomFamily instance", () => {
 const releaseIdx = atoms.indexOf(
  "function tryReleaseLiveThinkingAfterHistory",
 );
