...
// 先注册解析钩子：桥沿用仓库约定的**无扩展名相对 import**（jiti 可解析，
// Node 原生 ESM 不可），测试侧补一层解析。
import "./helpers/tsResolveHook.mjs";

// 用绝对 file URL 引入桥的源文件：相对路径在 `node --test` 下易受 cwd 影响。
const BASE = new URL("../resources/extensions/", import.meta.url).href;

const themeMod = await import(`${BASE}pi-deck-gui-bridge-theme.ts`);
const serializeMod = await import(`${BASE}pi-deck-gui-bridge-serialize.ts`);
const runtimeMod = await import(`${BASE}pi-deck-gui-bridge-runtime.ts`);
const transportMod = await import(`${BASE}pi-deck-gui-bridge-transport.ts`);
const guiMod = await import(`${BASE}pi-deck-gui-bridge-gui.ts`);
...
describe("bridge serialize: 适配器", () => {
	...
	it("SelectList → select 节点，读得到私有状态", () => {
		const list = new tui.SelectList([{ value: "a", label: "Alpha" }, { value: "b", label: "Beta" }], 5, makeSelectTheme());
...
describe("bridge runtime: 事件回灌", () => {
	...
	it("input 用公开 setValue 而不是逐字符 handleInput", () => {
		let value = "";
		runtimeMod.replayEvent({ setValue: (v) => { value = v; } }, { type: "input", value: "hello" });
		assert.equal(value, "hello");
	});

	it("filter 调公开 setFilter", () => {
		let filter = "";
		runtimeMod.replayEvent({ setFilter: (f) => { filter = f; } }, { type: "filter", value: "ab" });
...
describe("bridge runtime: 拦截层", () => {
	...
	it("重设时旧组件 dispose 被调用（§12.4）", () => {
		...
		runtime.wrapUI(ui);
		let disposed = 0;
		ui.setFooter(() => ({ render: () => ["a"], dispose: () => { disposed += 1; } }));
	...
	it("undefined 清除时也 dispose", () => {
		...
		runtime.wrapUI(ui);
		let disposed = 0;
		ui.setHeader(() => ({ render: () => ["a"], dispose: () => { disposed += 1; } }));
		ui.setHeader(undefined);
...
describe("bridge gui: ctx.gui 落点与校验", () => {
	...
	it("白名单含 §7.1-B 全部 14 个方法", () => {
		const methods = Object.keys(guiMod.GUI_SLOT_METHODS);
		assert.equal(methods.length, 14, `应有 14 个 GUI 专属位置，实际 ${methods.length}`);
		for (const expected of [
			"setSidebarPanel", "setSidebarSection", "setContentView", "setComposerToolbar",
			"setTitlebarAction", "setBanner", "setToolExtra", "setMessageExtra",
			"setThinkingExtra", "setDialogAction", "setDialogBody", "setSettingsSection",
			"setSessionItemExtra", "setContextMenuItem",
		]) {
	...
	it("undefined 移除贡献并推 null", () => {
		...
		assert.equal(guiMod.guiContributionCount(runtime), 0);
		const updates = pushed.filter((u) => u.type === "ui-update" && u.targetId === "gui:sidebar.panel:p");
		assert.equal(updates[updates.length - 1].node, null);
	});
	...
	it("order 缺省 1000，非法 order 回落默认", () => {
		...
		const update = pushed.filter((u) => u.type === "ui-update" && u.targetId === "gui:sidebar.panel:x").pop();
		assert.equal(update.node.slot.order, 1000, "缺省 order 应为 1000");
		gui.setSidebarPanel("y", () => ({ kind: "text", id: "2", text: "Y" }), { order: Number.NaN });
		const update2 = pushed.filter((u) => u.type === "ui-update" && u.targetId === "gui:sidebar.panel:y").pop();
	...
	it("order 与 title 被带上", () => {
		...
		const update = pushed.filter((u) => u.type === "ui-update" && u.targetId === "gui:sidebar.panel:z").pop();
	...
	it("isValidGuiNode 拒绝函数 / null / 字符串 / cyclic / React 元素", () => {
		assert.equal(guiMod.isValidGuiNode(() => {}), false);
	...
	it("React 元素返回值 → 贡献隐藏，不推内容", () => {
		...
		const updates = pushed.filter((u) => u.type === "ui-update" && u.targetId === "gui:sidebar.panel:react");
	...
	it("GuiComponent（有 render）也被接受", () => {
		...
		const update = pushed.filter((u) => u.type === "ui-update" && u.targetId === "gui:sidebar.panel:comp").pop();
	...
	it("render() 抛错 → 该贡献隐藏，不崩桥", () => {
		const { runtime, pushed } = makeGui();
		const gui = guiMod.createGuiNamespace(runtime);
		gui.setSidebarPanel("boom", () => ({ render: () => { throw new Error("render boom"); } }));
		const updates = pushed.filter((u) => u.type === "ui-update" && u.targetId === "gui:sidebar.panel:boom");
		assert.equal(updates[updates.length - 1].node, null);
	});
	...
	it("onPress 回调不序列化，只推 actionId", () => {
		...
		const update = pushed.filter((u) => u.type === "ui-update" && u.targetId === "gui:sidebar.panel:btn").pop();
		assert.ok(!JSON.stringify(update.node).includes("function"), "节点树不得包含函数");
		let pressed = 0;
		const actionId = guiMod.registerAction(() => { pressed += 1; });
	...
	it("toast 的 actions 拿到可回灌的 actionId", () => {
		...
		const gui = guiMod.createGuiNamespace(runtime);
		let clicked = 0;
		gui.toast("完成", { tone: "success", actions: [{ label: "撤销", onPress: () => { clicked += 1; } }] });
	...
	it("卸载时旧组件 dispose 被调用", () => {
		...
		const gui = guiMod.createGuiNamespace(runtime);
		let disposed = 0;
		gui.setSidebarPanel("d", () => ({ render: () => ({ kind: "text", id: "1", text: "D" }), dispose: () => { disposed += 1; } }));
		guiMod.clearGuiContributions(runtime, "unknown");
		assert.equal(disposed, 1);
	});
