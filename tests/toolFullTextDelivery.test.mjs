...
test("delivery strips redundant meta.result from tool messages", () => {
  ...
  assert.match(
    agentUtils,
    /export function stripToolResultForDelivery\(messages: ChatMessage\[\]\)/,
  );
  assert.match(agentUtils, /delete meta\.result;/);
  // 全部下发出口统一剥离：flush 两个分支 + getMessageWindow + disk 分页 facade
  assert.match(
    agentUtils,
    /stripToolResultForDelivery\(all\.slice\(dirtyFrom\)\)/,
  );
  assert.match(
    agentUtils,
    /stripToolResultForDelivery\(all\.slice\(boundedWindow\)\)/,
  );
  assert.match(
    agentManager,
    /stripToolResultForDelivery\(\[\s*\.\.\.summaryCards,\s*\.\.\.all\.slice\(windowStart\),?\s*\]\)/,
  );
...
test("full text read falls back to session file with LRU cache", () => {
  ...
  assert.match(
    agentManager,
    /this\.sessionHistoryReader\.readMessageFullText\(\s*sessionPath,\s*messageId,\s*entryId,?\s*\)/,
  );
...
test("ToolCard shows on-demand full-output entry with loading/error states", () => {
  ...
  assert.match(
    toolCard,
    /isTruncated = props\.message\.meta\?\.truncated === true/,
  );
