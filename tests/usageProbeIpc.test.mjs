...
const userUsageProbes = readFileSync(
...
test("主进程三个 handler 都注册且先校验后动作", () => {
  ...
  const getHandler = systemIpc.slice(
    getStart,
    systemIpc.indexOf("ipcMain.handle(", getStart),
  );
  ...
  const saveHandler = systemIpc.slice(
    saveStart,
    systemIpc.indexOf("ipcMain.handle(", saveStart),
  );
  ...
  const testHandler = systemIpc.slice(
    testStart,
    systemIpc.indexOf("ipcMain.handle(", testStart),
  );
...
test("主进程写入路径固定在 configDir（禁止拼接渲染层传入路径）", () => {
  ...
  assert.match(
    systemIpc,
    /saveUsageProbeForProvider\(\s*configManager\.getUsageProbeConfigDir\(backend\)/,
  );
...
test("preload 暴露与 previewApi stub 三处同步", () => {
  ...
  assert.match(
    previewApi,
    /getUsageProbes: async \(\) => \(\{\s*recognized: null/,
  );
...
test("backend 维度贯通：DSH 链路 = $DSH_HOME/.pideck 配置 + DSH 凭据库", () => {
  ...
  assert.match(
    configManager,
    /usageProbeSettingsDir\(backend: UsageProbeBackend\)/,
  );
  ...
  assert.match(
    configManager,
    /readCredential\(credentialRefFor\(\{\}, provider\)\)/,
  );
  ...
  assert.match(
    mainIndex,
    /resolveDshHomeDir\(settingsStore\.get\(\)\.dshHomeDir/,
  );
  assert.match(mainIndex, /\.credentials\.yaml/);
  // 渲染层：DSH 卡片用量行走 dsh backend；缓存 key 带 dsh: 前缀防与 pi 同名 provider 串号。
  const dshCards = readFileSync(
    "src/renderer/src/config/DshProviderCards.tsx",
    "utf8",
  );
  assert.match(dshCards, /backend="dsh"/);
  const inlineSource = readFileSync(
    "src/renderer/src/components/app/ProviderUsageInline.tsx",
    "utf8",
  );
  ...
  assert.match(
    inlineSource,
    /useProviderUsageEntry\(props\.provider, backend\)/,
  );
  assert.match(
    inlineSource,
    /useProviderUsageState\(props\.provider, backend\)/,
  );
  // 缓存 key 只在渲染层 atom 内使用；**发送给主进程的必须是原始 provider 名**——
  // 曾把 `dsh:deepseek` 整体当 provider 寄回主进程，DSH 卡片永远解析不出、用量为空。
  const usageHook = readFileSync(
    "src/renderer/src/hooks/useProviderUsage.ts",
    "utf8",
  );
  ...
  const dshEndpoint = readFileSync(
    "src/main/config/dshUsageEndpoint.ts",
    "utf8",
  );
  assert.match(dshEndpoint, /normalizeDshDeepseekProvider/);
  const usageCacheKeySource =
    usageHook.match(/export function usageCacheKey\([\s\S]*?\n\}/)?.[0] ?? "";
...
test("ConfigManager 用量查询按 provider 名路由：门控 + 识别 + 测试共用探测层", () => {
  ...
  assert.match(
    configManager,
    /async fetchProviderUsage\(provider: string, backend: UsageProbeBackend = "pi"\)/,
  );
  assert.match(
    configManager,
    /async getUsageProbeSettings\(\s*provider: string,\s*backend: UsageProbeBackend/,
  );
  assert.match(
    configManager,
    /async recognizeUsageTemplate\(\s*provider: string,\s*backend: UsageProbeBackend/,
  );
