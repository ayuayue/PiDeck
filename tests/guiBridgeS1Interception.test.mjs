...
/** 建桥运行时 + 记录推送的 transport。 */
async function makeBridge() {
	const runtimeMod = await import("../resources/extensions/pi-deck-gui-bridge-runtime.ts");
	const pushed = [];
...
maybeDescribe("S1: ctx.ui 拦截（真实 ExtensionRunner）", () => {
	...
	it("★ 桥包装一次后，另一个扩展取到的 ctx.ui 已是包装版", async () => {
		...
		assert.ok(pushed.some((u) => u.type === "ui-update" && u.targetId === "header" && u.node), "扩展 B 的 setHeader 必须落到桥");
		assert.ok(pushed.some((u) => u.type === "status" && u.key === "ext-b"), "扩展 B 的 setStatus 必须落到桥");
		assert.ok(pushed.some((u) => u.type === "working" && u.message === "B working"), "setWorkingMessage 必须落到桥");
		assert.ok(pushed.some((u) => u.type === "thinking-label" && u.label === "B thinking"), "setHiddenThinkingLabel 必须落到桥");
		assert.ok(pushed.some((u) => u.type === "title" && u.title === "B title"), "setTitle 必须落到桥");
	});
	...
	it("只补不拆：已有能力仍走原路", async () => {
		...
		assert.ok(rpcUi.__seen.some((s) => s.method === "setWidget" && s.key === "lines"), "字符串 widget 必须走原路");

		// setStatus 必须仍转发 —— 否则 PiDeck 的 pideck:auto-title 自动标题会失效（§7.7）
		rpcUi.__seen.length = 0;
		ui.setStatus("pideck:auto-title", "Auto Title");
		assert.ok(rpcUi.__seen.some((s) => s.method === "setStatus" && s.key === "pideck:auto-title"), "setStatus 必须转发，否则自动标题失效");

		// notify / setEditorText 不应被桥触碰
		rpcUi.__seen.length = 0;
		ui.notify("hello", "info");
		assert.ok(rpcUi.__seen.some((s) => s.method === "notify"), "notify 应走原路");
		rpcUi.__seen.length = 0;
		ui.setEditorText("text");
		assert.ok(rpcUi.__seen.some((s) => s.method === "set_editor_text"), "setEditorText 应走原路");
	});
