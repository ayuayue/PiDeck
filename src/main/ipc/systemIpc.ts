...
import {
	probeAllMirrors,
	type MirrorHealthResult,
} from "../update/mirrorHealth";
import {
	ChangelogService,
	type ChangelogLanguage,
} from "../update/ChangelogService";
...
import {
	createNetDownloader,
	fetchDshRunnerNodeIndex,
} from "../dsh/runtime/dshRuntimeIo";
...
import {
	...
	modelsFromPiConfig,
	refreshModelCatalogStore,
	refreshModelList,
	resolveModelListReport,
} from "../pi/modelListCache";
...
function isMcpServerDefinition(value: unknown): value is McpServerDefinition {
	...
	const optionalNumber = (key: string) =>
		!(key in value) || value[key] === undefined || typeof value[key] === "number";
	return (
		["command", "cwd", "url", "socket", "bearerToken", "bearerTokenEnv"].every(
			optionalString,
		) &&
		...
		(!("lifecycle" in value) ||
			...
			["lazy", "eager", "keep-alive", "lazy-keep-alive"].includes(
		...
		(!("directTools" in value) ||
			value.directTools === undefined ||
			typeof value.directTools === "boolean" ||
			(Array.isArray(value.directTools) &&
				value.directTools.every((entry) => typeof entry === "string")))
	);
}
...
function isMcpConfigFile(value: unknown): value is McpConfigFile {
	...
	if (
		...
		!isUnknownRecord(value.settings)
	)
		return false;
...
export type SystemIpcDeps = {
	...
	configureSessionScannerWsl?: (
		env: import("../wsl/WslPaths").WslEnvironment,
	) => Promise<void>;
	...
	) => Promise<import("../../shared/types").PiProxyTestResult>;
	...
	setSessionCatalogIdentityContext?: (ctx: {
		wslDistro?: string;
		wslUser?: string;
	}) => void;
	...
	configureSkillManagerWsl?: (
		env: import("../wsl/WslPaths").WslEnvironment | null,
	) => void;
	...
	sessionCommandIpcError?: (
		error: import("../../shared/types").SessionCommandError,
	) => Error;
	...
	extensionManager?: {
		checkPiUpdate: () => Promise<
			import("../../shared/types").PiUpdateCheckResult
		>;
...
export function registerSystemIpc(deps: SystemIpcDeps): void {
	...
	const refreshPiModelCatalogs = async (): Promise<void> => {
		...
		await refreshModelList(piLocator, settingsStore, configManager).catch(
			() => undefined,
		);
	};
	...
	ipcMain.handle(
		ipcChannels.projectsListModels,
		async (_event, _projectId?: string) => {
			try {
	...
	ipcMain.handle(
		...
		async (_event, projectId: unknown, force: unknown) => {
			...
			try {
				...
				if (forceArg) {
					const catalogRefreshed = await refreshModelCatalogStore(
						piLocator,
						settingsStore,
					);
	...
	ipcMain.handle(
		...
		async (_event, distro: unknown, user: unknown) => {
			...
			if (process.platform !== "win32") {
				return {
					...
					piVersion: "",
					piPath: "",
					error: mainCopy("wsl.windowsOnly"),
				};
			}
			try {
				const { execFile } = await import("node:child_process");
				...
				const pi = await piLocator.checkWslInstallation(distro, user, {
					force: true,
				});
				return {
					ok: true,
	...
	ipcMain.handle(
		...
		async (
			_event,
			command: string,
		): Promise<import("../../shared/types").PiInstallExecResult> => {
			void appLogger.info("pi", "Executing install command", { command });
			try {
				const { execFile } = await import("node:child_process");
				const result = await new Promise<
					import("../../shared/types").PiInstallExecResult
				>((resolve) => {
	...
	ipcMain.handle(
		...
		async (): Promise<import("../../shared/types").NpmAvailabilityResult> => {
			try {
				const { execFile } = await import("node:child_process");
				const result = await new Promise<
					import("../../shared/types").NpmAvailabilityResult
				>((resolve) => {
					const isWin = process.platform === "win32";
					if (isWin) {
						execFile(
	...
	if (extensionManager) {
		ipcMain.handle(ipcChannels.piUpdateCheck, async () => {
			...
			void appLogger.info("pi", "Pi update check completed", {
		...
		ipcMain.handle(ipcChannels.piUpdate, async () => {
			...
			void appLogger.info("pi", "Pi update command completed", {
	...
	const resolveAppInfo = (): Promise<AppInfo> => {
		appInfoPromise ??= (async () => {
			...
			return {
				...
				releasesUrl:
					RELEASES_URL ??
					`https://github.com/${UPDATE_REPO_OWNER}/${UPDATE_REPO}/releases`,
	...
	ipcMain.handle(ipcChannels.appNetworkAddresses, () =>
		listWebNetworkAddresses(),
	);
	...
	ipcMain.handle(
		...
		async (_event, kind: unknown, version: unknown) => {
	...
	ipcMain.handle(
		ipcChannels.appCheckUpdateMirrors,
		async (): Promise<MirrorHealthResult[]> => {
	...
	ipcMain.handle(
		...
		async (): Promise<ProcessMetricsSnapshot> => {
			const agents: Array<
				Pick<
					AgentProcessMetric,
					"agentId" | "pid" | "kind" | "sessionId" | "sessionTitle" | "sessionTitles"
				>
	...
	ipcMain.handle(ipcChannels.stopAgent, async (_event, agentId: unknown) => {
		...
		if (!result.ok) {
			throw new Error(
				result.error.debugDetails ?? `failed to stop agent ${agentId}`,
			);
		}
	});
	...
	ipcMain.handle(
		...
		async (
			_event,
			options?: { target?: SessionRuntimeTarget; days?: number; limit?: number },
		) =>
	...
	ipcMain.handle(
		ipcChannels.rpcLogsSave,
		async (_event, options?: { entries?: unknown }) => {
	...
	ipcMain.handle(
		ipcChannels.rpcLogsClear,
		async (_event, target?: SessionRuntimeTarget) =>
			rpcLogger.clear(resolveRpcRuntimeAgent(target)),
	);
	ipcMain.handle(
		ipcChannels.rpcLoggingSet,
		async (_event, target: SessionRuntimeTarget, enabled: boolean) => {
	...
	ipcMain.handle(
		...
		async (_event, url: string, forceSystem?: boolean) => {
		...
		async (
			...
			try {
				...
				if (!result) {
					void appLogger.info(
						"changelog",
						"All sources failed and no cached copy exists",
						{ lang },
					);
					return {
						markdown: null,
	...
	let runnerNodeInstallInflight: Promise<
		import("../../shared/types/dshRunnerNode").DshRunnerNodeInstallResult
	> | null = null;
	ipcMain.handle(ipcChannels.dshInstallRunnerNode, async () => {
		...
		runnerNodeInstallInflight = (async () => {
			...
			const result = await installDshRunnerNodeSidecar({
				...
				indexUrl:
					process.env.DSH_RUNNER_NODE_INDEX_URL || settings.dshRunnerNodeIndexUrl,
			...
			if (result.ok && restartDshHost && dshHostIsStarted?.()) {
				void restartDshHost().catch((error) => {
					void appLogger.warn(
						"dsh",
						"Failed to restart DSH host after installing runner node",
						{
							error: error instanceof Error ? error.message : String(error),
						},
					);
	...
	ipcMain.handle(
		...
		async (_event, patch: Partial<AppSettings>) => {
			...
			if (
				"dshRunnerNodePath" in patch &&
				prevSettings.dshRunnerNodePath !== settings.dshRunnerNodePath
			) {
				if (restartDshHost && dshHostIsStarted?.()) {
					void restartDshHost().catch((error) => {
						void appLogger.warn(
							"dsh",
							"Failed to restart DSH host after runner node path change",
							{
								error: error instanceof Error ? error.message : String(error),
							},
						);
			...
			if ("developerDiagnostics" in patch && diagnosticsMonitor) {
				void diagnosticsMonitor
					.setEnabled(settings.developerDiagnostics)
					.catch((error) => {
						void appLogger.warn(
							"diagnostics",
							"Failed to toggle developer diagnostics",
							{
								error: error instanceof Error ? error.message : String(error),
							},
						);
			...
			if ("wslEnabled" in patch || "wslDistro" in patch || "wslUser" in patch) {
				...
				if (
					...
					const environment = await resolveWslEnvironment(
						settings.wslDistro,
						settings.wslUser,
						{
							warn: (msg: string, detail: unknown) =>
								console.warn("[PiDeck] " + String(msg), detail),
						},
					);
					if (configureSessionScannerWsl)
						await configureSessionScannerWsl(environment);
					...
					if (configureExtensionManagerWsl)
						configureExtensionManagerWsl(environment);
					if (configureConfigManagerWsl) configureConfigManagerWsl(environment);
					if (configureXuePromptManagerWsl)
						configureXuePromptManagerWsl(environment);
	...
	ipcMain.handle(
		...
		async (_event, path: string, enabled: boolean) => {
	...
	ipcMain.handle(
		...
		async (_event, skillPath: unknown, newName: unknown) => {
			// 渲染层入参不可信：先校验再进 SkillManager（与 projectResourceIpc 的边界校验同一纪律）。
			if (
				typeof skillPath !== "string" ||
				skillPath.trim().length === 0 ||
				skillPath.length > 4096
			) {
	...
	ipcMain.handle(
		...
		async (_event, direction: unknown) => {
			if (direction !== "pi-to-dsh" && direction !== "dsh-to-pi") {
				throw new Error("invalid migration direction");
			}
			if (!providerMigration)
				throw new Error("provider migration is not available");
			return previewProviderMigration(
				providerMigration,
				direction as ProviderMigrationDirection,
			);
		},
	);
	ipcMain.handle(
		ipcChannels.configApplyProviderMigration,
		async (_event, direction: unknown, provider: unknown) => {
			if (direction !== "pi-to-dsh" && direction !== "dsh-to-pi") {
				throw new Error("invalid migration direction");
			}
			...
			if (!providerMigration)
				throw new Error("provider migration is not available");
			const result = await applyProviderMigration(
				providerMigration,
				direction as ProviderMigrationDirection,
				provider,
			);
	...
	ipcMain.handle(ipcChannels.configGetMcp, (_event, projectId?: unknown) => {
		...
		return configManager.getMcpConfig(
			projectResourceManager.getProjectRoot(projectId.trim()),
		);
	});
	ipcMain.handle(ipcChannels.configSaveMcp, async (_event, data: unknown) => {
		if (!isMcpConfigFile(data)) {
			return {
				valid: false,
				error: "mcp.json must contain an object of server definitions",
			};
		}
	...
	ipcMain.handle(
		ipcChannels.configProbeMcp,
		async (_event, definition: unknown) => {
	...
	ipcMain.handle(ipcChannels.configSaveModels, async (_event, data) => {
		...
		return {
			...
			modelLoadDetail:
				localModels.length > 0 ? "" : "no models in saved models.json",
		};
	});
	ipcMain.handle(ipcChannels.configSaveAuth, async (_event, data) => {
		...
		void appLogger.info("config", "Auth config saved", {
			authCount: Object.keys(data ?? {}).length,
		});
		return result;
	});
	ipcMain.handle(ipcChannels.configSaveSettings, async (_event, settings) => {
		...
		void appLogger.info("config", "Pi settings config saved", {
	...
	ipcMain.handle(
		...
		async (_event, packageJson: string) => {
	...
	ipcMain.handle(
		...
		async (
			...
			const proxyTarget = resolveConfigProxyTarget(
				settingsStore.get(),
				asConfigProxyMode(payload?.proxyMode),
			);
	...
	ipcMain.handle(
		ipcChannels.configGetTokendanceModels,
		async (_event, force: unknown) => {
	...
	ipcMain.handle(
		ipcChannels.configTokendanceAuthStart,
		async (_event, payload: unknown) => {
			// 未装配 = 主进程没注册授权能力（预览/测试壳），返回失败不抛异常。
			if (!tokendanceAuth)
				return { ok: false, error: "TokenDance auth unavailable" };
			const mode =
				typeof payload === "object" && payload
					? (payload as { mode?: unknown }).mode
					: undefined;
			// 枚举白名单：只认 headless，其余（含缺失/非法）一律取默认 callback。
			const requested: TokendanceAuthMode =
				mode === "headless" ? "headless" : "callback";
			try {
				// callback 模式要绑定本地端口（异步），失败时 store 内部降级为 headless 并带 fallbackReason。
				return {
					ok: true,
					...(await tokendanceAuth.start({ mode: requested })),
				} as const;
			} catch (error) {
	...
	ipcMain.handle(
		ipcChannels.configTokendanceAuthAwait,
		async (_event, payload: unknown) => {
			...
			void appLogger.warn("config", "TokenDance callback auth failed", {
				error: result.error,
			});
	...
	ipcMain.handle(
		ipcChannels.configTokendanceAuthCancel,
		async (_event, payload: unknown) => {
			...
			const flowId =
				typeof payload === "object" && payload
					...
					: undefined;
			if (typeof flowId !== "string" || !flowId)
				return { ok: false, error: "Invalid auth cancel input" };
			tokendanceAuth?.cancel(flowId);
			return { ok: true } as const;
		},
	);
	ipcMain.handle(
		ipcChannels.configTokendanceAuthExchange,
		async (_event, payload: unknown) => {
			...
			void appLogger.warn("config", "TokenDance auth exchange failed", {
				error: result.error,
			});
			return { ok: false, error: result.error } as const;
		},
	...
	ipcMain.handle(
		ipcChannels.configInstallTokendance,
		async (_event, payload: unknown) => {
			...
			if (!tokendanceInstall) {
				return {
					...
					piSaved: false,
					dshSaved: false,
					error: "TokenDance install unavailable",
				};
			}
			const apiKey =
			...
			try {
				...
				return {
					...
					piSaved: false,
					dshSaved: false,
					error: "TokenDance install failed",
				};
			}
		},
	);
	ipcMain.handle(
		...
		async (_event, payload: unknown) => {
			...
			try {
				...
				if (settingsJson)
					await writeFile(join(tempDir, "settings.json"), settingsJson, "utf8");
				...
				const result = await probePiModel(
					...
					resolveConfigProxyTarget(
						settingsStore.get(),
						asConfigProxyMode(raw.proxyMode),
					),
					{ [PROBE_AGENT_DIR_ENV]: agentDirEnv },
				);
				void appLogger.info(
					"config",
					"Provider connection tested via pi (isolated)",
					{
						providerName,
						modelId,
						success: result.success,
	...
	ipcMain.handle(
		...
		async (_event, payload: { provider: string; backend?: "pi" | "dsh" }) => {
	...
	ipcMain.handle(
		...
		async (_event, payload: unknown) => {
			const raw =
				payload && typeof payload === "object"
					? (payload as { provider?: unknown; backend?: unknown })
					: {};
	...
	ipcMain.handle(
		...
		async (_event, payload: unknown) => {
			...
			const template =
				typeof input.template === "string" ? input.template.trim() : undefined;
			if (
				template &&
				template !== "general" &&
				template !== "newapi" &&
				...
				const knownBuiltin = USAGE_PROBE_CANDIDATES.some(
					(c) => c.templateId === template,
				);
				if (!knownBuiltin) {
			...
			const result = await configManager.testUsageProbe({
				...
				...(typeof input.cookiePath === "string"
					...
					: {}),
				...(typeof input.valuePath === "string"
					? { valuePath: input.valuePath }
					: {}),
				...(typeof input.currencyPath === "string"
	...
	ipcMain.handle(ipcChannels.configInstallUsageSkill, async () => {
		...
		if (result.success) {
			void appLogger.info("skill", "Usage probe skill template installed", {
				path: result.path,
			});
	...
	ipcMain.handle(ipcChannels.configInstallImageGenSkill, async () => {
		...
		if (result.success) {
			void appLogger.info("skill", "Image-gen skill template installed", {
				path: result.path,
			});
			return { success: true, path: result.path };
		}
		void appLogger.warn("skill", "Failed to install image-gen skill template", {
			error: result.error,
		});
		return { success: false, error: result.error };
	});
	...
	ipcMain.handle(ipcChannels.appToggleDevTools, () =>
		toggleMainWindowDevTools(getMainWindow()),
	);
}
