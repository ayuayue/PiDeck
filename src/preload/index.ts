...
const api = {
	...
	editors: {
		list: () =>
			ipcRenderer.invoke(ipcChannels.editorsList) as Promise<ExternalEditor[]>,
		redetect: () =>
		...
		chooseExecutable: () =>
			ipcRenderer.invoke(ipcChannels.editorsChooseExecutable) as Promise<
				string | null
			>,
	...
	projects: {
		...
		reorder: (projectIds: string[]) =>
			ipcRenderer.invoke(ipcChannels.projectsReorder, projectIds) as Promise<
				Project[]
			>,
		// 重命名项目显示名（仅改 label，不动磁盘目录）；返回更新后的项目列表
		rename: (id: string, name: string) =>
			ipcRenderer.invoke(ipcChannels.projectsRename, id, name) as Promise<
				Project[]
			>,
		...
		chooseChatPath: () =>
			ipcRenderer.invoke(ipcChannels.projectsChooseChatPath) as Promise<
				string | null
			>,
		// 设置聊天记录目录
		...
		listModelsReport: (projectId?: string, force?: boolean) =>
			ipcRenderer.invoke(
				ipcChannels.projectsListModelsReport,
				projectId,
				force,
			) as Promise<ModelListReport>,
		...
		) => subscribe(ipcChannels.projectsTrustRequest, callback),
		respondTrustRequest: (
			requestId: string,
	...
	projectResources: {
		list: (projectId: string) =>
			ipcRenderer.invoke(
				ipcChannels.projectResourcesList,
				projectId,
			) as Promise<ProjectResourceListResult>,
		...
		toggleInherited: (input: ProjectInheritedResourceToggleInput) =>
			ipcRenderer.invoke(
				ipcChannels.projectResourcesToggleInherited,
				input,
			) as Promise<ProjectResourceOverrides>,
		...
		discovery: (projectId: string) =>
			ipcRenderer.invoke(
				ipcChannels.projectResourcesDiscovery,
				projectId,
			) as Promise<ProjectResourceDiscoveryResult>,
	},
	files: {
		list: (
			projectId: string,
		...
		detectFileManager: () =>
			ipcRenderer.invoke(
				ipcChannels.filesDetectFileManager,
			) as Promise<FileManagerInfo | null>,
		/** 在系统文件管理器中打开目录 */
		...
		pathsExist: (paths: string[], scope?: ProjectFileAccessScope) =>
			ipcRenderer.invoke(ipcChannels.filesPathsExist, paths, scope) as Promise<
				boolean[]
			>,
		...
		copy: (sourcePaths: string[], targetDir: string) =>
			ipcRenderer.invoke(ipcChannels.filesCopy, sourcePaths, targetDir) as Promise<
				string[]
			>,
		/** 移动来源路径到目标目录（同设备 rename，跨设备 cp+rm） */
		move: (sourcePaths: string[], targetDir: string) =>
			ipcRenderer.invoke(ipcChannels.filesMove, sourcePaths, targetDir) as Promise<
				string[]
			>,
		 ...
		 */
		getClipboardPaths: () =>
			clipboardSync<string[]>(ipcChannels.clipboardReadFilePaths, []),
	},
	pasteFiles: {
		...
		write: (input: PasteFileWriteInput) =>
			ipcRenderer.invoke(
				...
				input,
			) as Promise<PasteFileWriteResult>,
		/** 移除 chip 时同步删除落盘文件（仅限受管目录内路径）。 */
		delete: (path: string) =>
			ipcRenderer.invoke(ipcChannels.pasteFilesDelete, path) as Promise<void>,
		/** 启动清理过期粘贴文件（渲染层一般不调用）。 */
		cleanup: () =>
			ipcRenderer.invoke(ipcChannels.pasteFilesCleanup) as Promise<number>,
		/** 设置页占用统计：userData 新文件 + 各项目遗留 .pideck-paste。 */
		getSize: () =>
			ipcRenderer.invoke(ipcChannels.pasteFilesGetSize) as Promise<number>,
		/** 设置页一键清空两个受管根下的 paste-* 文件。 */
		clearAll: () =>
			ipcRenderer.invoke(ipcChannels.pasteFilesClearAll) as Promise<number>,
	},
	dialog: {
		/**
		...
		pickFiles: (options?: { title?: string; includeDirectories?: boolean }) =>
			ipcRenderer.invoke(ipcChannels.dialogPickFiles, options) as Promise<
				string[]
			>,
	...
	sessions: {
		...
		onCatalogRefreshed: (listener: (input: { projectId: string }) => void) => {
			const handler = (_event: unknown, payload: { projectId: string }) =>
				listener(payload);
		...
		discoverDshModels: (input: DshModelDiscoveryInput) =>
			ipcRenderer.invoke(ipcChannels.dshDiscoverModels, input) as Promise<
				FetchedModel[]
			>,
		...
		detectDshRunnerNode: (configuredPath?: string) =>
			ipcRenderer.invoke(
				ipcChannels.dshDetectRunnerNode,
				configuredPath,
			) as Promise<DshRunnerNodeInfo>,
		/** 打开文件选择框挑 node.exe；取消返回 null。 */
		chooseDshRunnerNode: () =>
			ipcRenderer.invoke(ipcChannels.dshChooseRunnerNode) as Promise<
				string | null
			>,
		/** 下载 Node 24 到应用数据目录（不改系统 PATH）。 */
		installDshRunnerNode: () =>
			ipcRenderer.invoke(
				ipcChannels.dshInstallRunnerNode,
			) as Promise<DshRunnerNodeInstallResult>,
		/**
		...
		onDshRuntimeInstallProgress: (
			callback: (progress: DshRuntimeInstallProgress) => void,
		...
		readDshCredential: (ref: string) =>
			ipcRenderer.invoke(ipcChannels.dshCredentialRead, ref) as Promise<
				string | undefined
			>,
		...
		listArchived: () =>
			ipcRenderer.invoke(ipcChannels.sessionsCatalogListArchived) as Promise<
				import("../shared/types").ArchivedPiSession[]
			>,
		...
		readRecordMessages: (sessionId: string) =>
			ipcRenderer.invoke(
				...
				sessionId,
			) as Promise<import("../shared/types").ChatMessage[]>,
		readRecordMessagePage: (
			sessionId: string,
			before?: number,
			pageSize?: number,
			options?: { beforeEntryId?: string },
		) =>
			ipcRenderer.invoke(
		...
		prepareCatalogResend: (
			...
			ipcRenderer.invoke(
				...
				SessionCommandResult<{ text: string; images?: ImageContent[] }>
			>,
		...
		readProcessEvents: (sessionId: string) =>
			ipcRenderer.invoke(
				...
				sessionId,
			) as Promise<SessionProcessEvent[]>,
		...
		listSessionFileChanges: (sessionId: string) =>
			ipcRenderer.invoke(
				...
				sessionId,
			) as Promise<import("../shared/types").SessionFileChange[]>,
		/** 会话级 todo 快照（pi-deck-todo custom 条目重建，历史会话任务 tab）。 */
		listSessionTodo: (sessionId: string) =>
			ipcRenderer.invoke(
				ipcChannels.sessionsListSessionTodo,
				sessionId,
			) as Promise<import("../shared/types").SessionTodoSnapshot | undefined>,
		/** DSH 技能目录（skill.list 只读；/name 斜杠调用，G7）。 */
		listDshSkills: (agentId: string) =>
			ipcRenderer.invoke(ipcChannels.dshListSkills, agentId) as Promise<
				import("../shared/types").DshSkillView[]
			>,
		...
		importDshForeignSession: (dshSessionId: string) =>
			ipcRenderer.invoke(
				...
				dshSessionId,
			) as Promise<import("../shared/types").SessionRecord>,
		/** DSH 外部会话全量同步（自动发现：catalog 未映射的 host 根会话全部导入）。 */
		syncDshForeignSessions: () =>
			ipcRenderer.invoke(ipcChannels.dshSyncForeignSessions) as Promise<{
				imported: number;
				skipped: number;
			}>,
		/** DSH 归档区会话清单（G14：恢复入口用；目录已移入 .pideck-archive 的 host 会话，含标题）。 */
		listArchivedDshSessions: () =>
			ipcRenderer.invoke(ipcChannels.dshListArchived) as Promise<
				import("../shared/types").ArchivedDshSession[]
			>,
		...
		listDshDynamicPlugins: () =>
			ipcRenderer.invoke(ipcChannels.dshPluginList) as Promise<
				import("../shared/types").DshPluginView[]
			>,
		/** DSH 静态 Loader 条目清单（origin 标注 user/builtin 来源）。 */
		listDshStaticPlugins: () =>
			ipcRenderer.invoke(ipcChannels.dshPluginStaticList) as Promise<
				import("../shared/types").DshStaticPluginView[]
			>,
		/** DSH 用户自装静态插件卸载（移除用户补丁层行 + 可选回收插件目录；host 重启后生效）。 */
		uninstallDshUserPlugin: (
			input: import("../shared/types").DshUserPluginUninstallInput,
		) =>
			ipcRenderer.invoke(ipcChannels.dshPluginUserUninstall, input) as Promise<
				import("../shared/types").DshUserPluginUninstallResult
			>,
		...
		uninstallDshPlugin: (
			input: import("../shared/types").DshPluginLifecycleInput,
		) =>
			ipcRenderer.invoke(
		...
		sendPrompt: (input: SendSessionPromptInput) =>
			ipcRenderer.invoke(
				...
				input,
			) as Promise<SendSessionPromptResult>,
		...
		listRuntimes: () =>
			ipcRenderer.invoke(ipcChannels.sessionsRuntimeList) as Promise<
				SessionRuntimeInfo[]
			>,
		...
		restoreRewindCheckpoint: (
			...
			ipcRenderer.invoke(
				...
				SessionCommandResult<SessionTargetedValue<RewindRestoreResult>>
			>,
	...
	git: {
		...
		listRepos: (projectId: string) =>
			ipcRenderer.invoke(ipcChannels.gitListRepos, projectId) as Promise<
				GitRepoInfo[]
			>,
		...
		worktreeList: (projectId: string) =>
			ipcRenderer.invoke(ipcChannels.gitWorktreeList, projectId) as Promise<
				WorktreeEntry[]
			>,
		// 创建新的 worktree
		...
		refs: (projectId: string, repoPath?: string) =>
			ipcRenderer.invoke(ipcChannels.gitRefs, projectId, repoPath) as Promise<
				GitRef[]
			>,
		...
		status: (projectId: string, repoPath?: string) =>
			ipcRenderer.invoke(ipcChannels.gitStatus, projectId, repoPath) as Promise<
				import("../shared/types").GitResourceGroups
			>,
		...
		init: (projectId: string) =>
			ipcRenderer.invoke(ipcChannels.gitInit, projectId) as Promise<void>,
		/** Push：将当前分支推送到远程 */
		...
		chooseExecutable: () =>
			ipcRenderer.invoke(ipcChannels.gitChooseExecutable) as Promise<
				string | null
			>,
	},
	pi: {
		...
		checkUpdate: () =>
			ipcRenderer.invoke(
				ipcChannels.piUpdateCheck,
			) as Promise<PiUpdateCheckResult>,
		update: () =>
		...
		execInstall: (command: string) =>
			ipcRenderer.invoke(
				...
				command,
			) as Promise<PiInstallExecResult>,
		/** 检查 npm 是否可用 */
		checkNpm: () =>
	...
	wsl: {
		...
		validateConnection: (distro: string, user: string) =>
			ipcRenderer.invoke(
				ipcChannels.wslValidateConnection,
				distro,
				user,
			) as Promise<WslConnectionValidation>,
	},
	system: {
		/** 进程监控：拉取 Electron 各进程 + pi agent 子进程内存/CPU 快照 */
		getProcessMetrics: () =>
			ipcRenderer.invoke(
				ipcChannels.processMetrics,
	...
	logs: {
		list: (query?: AppLogQuery) =>
			ipcRenderer.invoke(ipcChannels.logsList, query ?? {}) as Promise<
				AppLogEntry[]
			>,
		listPage: (query?: AppLogQuery) =>
			ipcRenderer.invoke(
				ipcChannels.logsListPage,
				query ?? {},
			) as Promise<AppLogPage>,
		clear: () => ipcRenderer.invoke(ipcChannels.logsClear) as Promise<void>,
		openFolder: () =>
			ipcRenderer.invoke(ipcChannels.logsOpenFolder) as Promise<void>,
	...
	rpcLogs: {
		...
		get: (options?: {
			...
			ipcRenderer.invoke(ipcChannels.rpcLogsGet, options) as Promise<
				RpcLogEntry[]
			>,
		/** 实时查看弹窗初始历史：主进程环形缓冲（按 agentId 过滤） */
		getLive: (agentId?: string) =>
			ipcRenderer.invoke(ipcChannels.rpcLogsGetLive, agentId) as Promise<
				RpcLogEntry[]
			>,
		...
		onLog: (callback: (batch: RpcLogBatch) => void) =>
			subscribe<{ agentId: string; entries: RpcLogEntry[] }>(
				ipcChannels.agentsRpcLog,
				callback,
			),
	...
	app: {
		...
		networkAddresses: () =>
			ipcRenderer.invoke(ipcChannels.appNetworkAddresses) as Promise<
				WebNetworkAddress[]
			>,
		preferredSystemLanguages: () =>
			ipcRenderer.invoke(ipcChannels.appPreferredSystemLanguages) as Promise<
				string[]
			>,
		...
		getUpdateStatus: () =>
			...
			) as Promise<AppUpdateStatusSnapshot | null>,
		/** 记录已提示过的版本（每版本只提示一次）。 */
		notifyUpdateSeen: (kind: "app" | "pi", version: string) =>
			ipcRenderer.invoke(
				ipcChannels.appUpdateNotifySeen,
				kind,
				version,
			) as Promise<void>,
		/** 跳过某版本（该版本不再主动提示）。 */
		skipUpdateVersion: (version: string) =>
			ipcRenderer.invoke(
				ipcChannels.appUpdateSkipVersion,
				version,
			) as Promise<void>,
		/** 探测内置更新镜像可用性与速度（设置页「更新源」）。 */
		checkUpdateMirrors: () =>
			ipcRenderer.invoke(ipcChannels.appCheckUpdateMirrors) as Promise<
				MirrorHealthResult[]
			>,
		feedbackEnvironment: () =>
			ipcRenderer.invoke(
		...
		onShortcutTriggered: (
			callback: (id: import("../shared/shortcuts").ShortcutId) => void,
		...
		openDataDir: () =>
			ipcRenderer.invoke(ipcChannels.appOpenDataDir) as Promise<{
				ok: boolean;
				error?: string;
			}>,
		rendererLog: (
	...
	skills: {
		...
		readContent: (path: string) =>
			ipcRenderer.invoke(
				ipcChannels.skillsReadContent,
				path,
			) as Promise<SkillContentResult>,
	...
	prompts: {
		list: () =>
			ipcRenderer.invoke(
				ipcChannels.promptsList,
			) as Promise<PiPromptTemplateListResult>,
		create: (input: CreatePiPromptTemplateInput) =>
			ipcRenderer.invoke(
				ipcChannels.promptsCreate,
				input,
		...
		edit: (filePath: string, content?: string) =>
			ipcRenderer.invoke(ipcChannels.promptsEdit, filePath, content) as Promise<
				string | void
			>,
		listByProject: (projectId: string) =>
			ipcRenderer.invoke(
				ipcChannels.promptsListByProject,
				projectId,
			) as Promise<PiPromptTemplateListResult>,
		deleteFromProject: (projectId: string, name: string) =>
			ipcRenderer.invoke(
				ipcChannels.promptsDeleteInProject,
				projectId,
				name,
			) as Promise<void>,
		rename: (oldName: string, newName: string) =>
			ipcRenderer.invoke(
				ipcChannels.promptsRename,
				oldName,
				newName,
			) as Promise<PiPromptTemplateSummary>,
		renameInProject: (projectId: string, oldName: string, newName: string) =>
			ipcRenderer.invoke(
				ipcChannels.promptsRenameInProject,
				projectId,
				oldName,
				newName,
			) as Promise<PiPromptTemplateSummary>,
	...
	promptStore: {
		search: (
			query: string,
			options?: { limit?: number; type?: string; category?: string; tag?: string },
		) =>
			ipcRenderer.invoke(
				ipcChannels.promptStoreSearch,
				query,
				options,
			) as Promise<PromptStoreSearchResult>,
		get: (id: string) =>
			ipcRenderer.invoke(
				ipcChannels.promptStoreGet,
				id,
			) as Promise<PromptStoreItem>,
		import: (data: {
			title: string;
			description: string;
			content: string;
	...
	skillHub: {
		search: (
			...
			ipcRenderer.invoke(ipcChannels.skillHubSearch, {
				query,
				page,
				pageSize,
				sortBy,
				order,
			}) as Promise<import("../shared/types").SkillHubSearchResult>,
		detail: (slug: string) =>
			ipcRenderer.invoke(ipcChannels.skillHubDetail, slug) as Promise<
				import("../shared/types").SkillHubDetail | null
			>,
		install: (slug: string, projectId?: string) =>
			ipcRenderer.invoke(ipcChannels.skillHubInstall, slug, projectId) as Promise<
				import("../shared/types").SkillHubInstallResult
			>,
	},
	yaoPrompts: {
		list: (opts?: {
			...
			ipcRenderer.invoke(
				...
				opts,
			) as Promise<YaoPromptListResult>,
		detail: (slug: string, category: string) =>
			ipcRenderer.invoke(
				ipcChannels.yaoPromptsDetail,
				slug,
				category,
			) as Promise<YaoPromptDetailResult>,
	...
	extensions: {
		list: (forceRefresh?: boolean) =>
			ipcRenderer.invoke(
				ipcChannels.extensionsList,
				forceRefresh,
			) as Promise<PiExtensionListResult>,
		...
		removeBuiltIn: (source: string) =>
			ipcRenderer.invoke(
				ipcChannels.extensionsRemoveBuiltIn,
				source,
			) as Promise<void>,
		restoreBuiltIn: (source: string) =>
			ipcRenderer.invoke(
				ipcChannels.extensionsRestoreBuiltIn,
				source,
			) as Promise<void>,
		update: () =>
			ipcRenderer.invoke(
				ipcChannels.extensionsUpdate,
			) as Promise<PiCliUpdateResult>,
		updateOne: (source: string) =>
			ipcRenderer.invoke(
				ipcChannels.extensionsUpdateOne,
				source,
			) as Promise<PiCliUpdateResult>,
		catalog: (query: import("../shared/types").PiPackageCatalogQuery) =>
			ipcRenderer.invoke(ipcChannels.extensionsCatalog, query) as Promise<
				import("../shared/types").PiPackageCatalog
			>,
		// ── 内置扩展热更新（版本号不跟应用版本走；检测走 AtomGit，更新写 userData 覆盖层）──
		builtInStatus: () =>
			ipcRenderer.invoke(
				ipcChannels.extensionsBuiltInUpdateStatus,
			) as Promise<BuiltInExtensionsUpdateStatus>,
		builtInCheck: (branch?: "main" | "dev") =>
			ipcRenderer.invoke(
				ipcChannels.extensionsBuiltInUpdateCheck,
				branch,
			) as Promise<BuiltInExtensionsCheckResult>,
		builtInUpdate: (branch?: "main" | "dev") =>
			ipcRenderer.invoke(
				ipcChannels.extensionsBuiltInUpdateApply,
				branch,
			) as Promise<BuiltInExtensionsUpdateResult>,
		builtInRestore: () =>
			ipcRenderer.invoke(
				ipcChannels.extensionsBuiltInUpdateRestore,
			) as Promise<BuiltInExtensionsUpdateResult>,
		builtInRestorePrevious: () =>
			ipcRenderer.invoke(
				ipcChannels.extensionsBuiltInUpdateRestorePrevious,
			) as Promise<BuiltInExtensionsUpdateResult>,
	...
	contentStore: {
		promptsStatus: () =>
			ipcRenderer.invoke(
				ipcChannels.promptsStoreUpdateStatus,
			) as Promise<BuiltinContentUpdateStatus>,
		promptsCheck: (branch?: "main" | "dev") =>
			ipcRenderer.invoke(
				ipcChannels.promptsStoreUpdateCheck,
				branch,
			) as Promise<BuiltinContentCheckResult>,
		promptsUpdate: (branch?: "main" | "dev") =>
			ipcRenderer.invoke(
				ipcChannels.promptsStoreUpdateApply,
				branch,
			) as Promise<BuiltinContentUpdateResult>,
		promptsRestore: () =>
			ipcRenderer.invoke(
				ipcChannels.promptsStoreUpdateRestore,
			) as Promise<BuiltinContentUpdateResult>,
		promptsRestorePrevious: () =>
			ipcRenderer.invoke(
				ipcChannels.promptsStoreUpdateRestorePrevious,
			) as Promise<BuiltinContentUpdateResult>,
		...
		skillsStatus: () =>
			ipcRenderer.invoke(
				ipcChannels.skillsStoreUpdateStatus,
			) as Promise<BuiltinContentUpdateStatus>,
		skillsCheck: (branch?: "main" | "dev") =>
			ipcRenderer.invoke(
				ipcChannels.skillsStoreUpdateCheck,
				branch,
			) as Promise<BuiltinContentCheckResult>,
		skillsUpdate: (branch?: "main" | "dev") =>
			ipcRenderer.invoke(
				ipcChannels.skillsStoreUpdateApply,
				branch,
			) as Promise<BuiltinContentUpdateResult>,
		skillsRestore: () =>
			ipcRenderer.invoke(
				ipcChannels.skillsStoreUpdateRestore,
			) as Promise<BuiltinContentUpdateResult>,
		skillsRestorePrevious: () =>
			ipcRenderer.invoke(
				ipcChannels.skillsStoreUpdateRestorePrevious,
			) as Promise<BuiltinContentUpdateResult>,
	...
	settings: {
		...
		webServiceStatus: () =>
			ipcRenderer.invoke(
				ipcChannels.webServiceStatus,
			) as Promise<WebServiceStatusInfo>,
		testPiProxy: () =>
			ipcRenderer.invoke(
	...
	security: {
		...
		updateConfig: (patch: Partial<SecurityConfig>) =>
			ipcRenderer.invoke(ipcChannels.securityUpdateConfig, patch) as Promise<
				{ ok: true; config: SecurityConfig } | { ok: false; error: string }
			>,
	...
	config: {
		previewProviderMigration: (
			...
			ipcRenderer.invoke(
				ipcChannels.configPreviewProviderMigration,
				direction,
			) as Promise<
		...
		applyProviderMigration: (
			...
			ipcRenderer.invoke(
				ipcChannels.configApplyProviderMigration,
				direction,
				provider,
			) as Promise<
		...
		onModelsVerifyResult: (callback: (payload: ModelsVerifyResult) => void) =>
			subscribe<ModelsVerifyResult>(
				ipcChannels.configModelsVerifyResult,
				callback,
			),
		...
		saveRaw: (fileName: string, rawJson: string) =>
			ipcRenderer.invoke(ipcChannels.configSaveRaw, fileName, rawJson) as Promise<{
				valid: boolean;
				error?: string;
			}>,
		export: () => ipcRenderer.invoke(ipcChannels.configExport) as Promise<string>,
		import: (packageJson: string) =>
			ipcRenderer.invoke(ipcChannels.configImport, packageJson) as Promise<{
				valid: boolean;
				error?: string;
			}>,
		...
		fetchModels: (
			...
			ipcRenderer.invoke(ipcChannels.configFetchModels, {
				baseUrl,
				apiKey,
				apiType,
				headers,
		...
		tokendanceAuthStart: (mode?: TokendanceAuthMode) =>
			ipcRenderer.invoke(ipcChannels.configTokendanceAuthStart, {
				mode,
			}) as Promise<
				| {
						ok: true;
		...
		/** 视觉桥：清空事件文件 */
		visionClearEvents: () =>
			ipcRenderer.invoke(ipcChannels.visionClearEvents) as Promise<{
				ok: boolean;
			}>,
		...
		testProvider: (
			...
			ipcRenderer.invoke(ipcChannels.configTestProvider, {
				providerName,
				modelId,
				provider,
				apiKey,
		...
		fetchUsage: (provider: string, backend?: "pi" | "dsh") =>
			ipcRenderer.invoke(ipcChannels.configFetchUsage, {
				provider,
				backend,
			}) as Promise<ProviderUsageResult>,
		/** 安装内置「用量查询自定义」技能模板到 ~/.pi/agent/skills/usage-probe */
		installUsageSkill: () =>
			ipcRenderer.invoke(ipcChannels.configInstallUsageSkill) as Promise<{
				success: boolean;
				path?: string;
				error?: string;
			}>,
		...
		getUsageProbes: (provider: string, backend?: "pi" | "dsh") =>
			ipcRenderer.invoke(ipcChannels.configGetUsageProbes, {
				provider,
				backend,
		...
		listUsageProbeStates: (
			payload: { providers?: string[]; backend?: "pi" | "dsh" } = {},
		) =>
			ipcRenderer.invoke(
		...
		installImageGenSkill: () =>
			ipcRenderer.invoke(ipcChannels.configInstallImageGenSkill) as Promise<{
				success: boolean;
				path?: string;
				error?: string;
			}>,
	...
	configBackups: {
		...
		create: (reason?: import("../shared/types/backup").ConfigBackupReason) =>
			ipcRenderer.invoke(
				ipcChannels.configBackupCreate,
				reason ?? "manual",
			) as Promise<import("../shared/types/backup").ConfigBackupActionResult>,
		/** 读取备份详情（文件内容已脱敏）。 */
		read: (id: string) =>
	...
	pet: {
		...
		getPendingFocusTarget: () =>
			ipcRenderer.invoke(
				ipcChannels.petGetFocusTargetPending,
			) as Promise<FocusTargetPayload | null>,
		...
		/** 右键上下文菜单 */
		contextMenu: () =>
			ipcRenderer.invoke(ipcChannels.petContextMenu) as Promise<void>,
	},
	sounds: {
		...
		listCustom: () =>
			ipcRenderer.invoke(ipcChannels.soundsListCustom) as Promise<
				CustomSoundInfo[]
			>,
		/** 弹选择框导入自定义音频；取消/非法/超限返回结构化错误 */
		importCustom: () =>
			ipcRenderer.invoke(
				ipcChannels.soundsImportCustom,
			) as Promise<SoundImportResult>,
		/** 删除自定义音频文件 */
	...
	announcements: {
		...
		list: () =>
			ipcRenderer.invoke(
				ipcChannels.announcementList,
			) as Promise<AnnouncementState>,
		/** 手动刷新公告（设置页/入口按钮；主进程防重入，并发调用安全） */
		refresh: () =>
			ipcRenderer.invoke(
				ipcChannels.announcementRefresh,
			) as Promise<AnnouncementState>,
		/** 标记单条公告已读（幂等） */
		markRead: (id: string) =>
	...
	feishu: {
		...
		disconnect: () =>
			ipcRenderer.invoke(ipcChannels.feishuDisconnect) as Promise<{
				success: boolean;
			}>,
		...
		statusRequest: () =>
			ipcRenderer.invoke(
				ipcChannels.feishuStatusRequest,
			) as Promise<FeishuBridgeStatus>,
		...
		botConfig: (botId: string, patch: Partial<FeishuBotConfig>) =>
			ipcRenderer.invoke(ipcChannels.feishuBotConfig, botId, patch) as Promise<
				FeishuBotConfig | undefined
			>,
		...
		testConnection: (appId: string, appSecret: string) =>
			ipcRenderer.invoke(
				ipcChannels.feishuTestConnection,
				appId,
				appSecret,
			) as Promise<FeishuTestResult>,
		bindingsList: () =>
			ipcRenderer.invoke(ipcChannels.feishuBindingsList) as Promise<
				FeishuChatBinding[]
			>,
		bindingRemove: (chatId: string) =>
			ipcRenderer.invoke(
				ipcChannels.feishuBindingRemove,
				chatId,
			) as Promise<boolean>,
		bindingUpdate: (chatId: string, patch: Partial<FeishuChatBinding>) =>
			ipcRenderer.invoke(
				ipcChannels.feishuBindingUpdate,
				chatId,
				patch,
			) as Promise<FeishuChatBinding | undefined>,
		...
		sessionBotGet: (sessionId: string) =>
			ipcRenderer.invoke(ipcChannels.feishuSessionBotGet, sessionId) as Promise<
				string | null
			>,
		sessionBotSet: (sessionId: string, botId: string | null) =>
			ipcRenderer.invoke(
				ipcChannels.feishuSessionBotSet,
				sessionId,
				botId,
			) as Promise<FeishuSessionBotResult>,
	},
	...
	scratchPad: {
		...
		load: (draftPath?: string) =>
			ipcRenderer.invoke(
				ipcChannels.scratchPadLoad,
				draftPath,
			) as Promise<ScratchPadData>,
	...
	imagegen: {
		generate: (request: ImageGenRequest) =>
			ipcRenderer.invoke(
				ipcChannels.imagegenGenerate,
				request,
			) as Promise<ImageGenResult>,
		getConfig: () =>
			ipcRenderer.invoke(
				ipcChannels.imagegenGetConfig,
			) as Promise<ImageGenConfigFile>,
		saveConfig: (config: ImageGenConfigFile) =>
			ipcRenderer.invoke(
				ipcChannels.imagegenSaveConfig,
				config,
			) as Promise<ImageGenSaveResult>,
		/** 按 blob 引用名取回落盘图片 base64（历史消息只带 ref，展示走 pideck-img://） */
		readImageBlob: (ref: string) =>
			ipcRenderer.invoke(
				ipcChannels.imagegenReadImageBlob,
				ref,
			) as Promise<ImageBlobPayload | null>,
	},

	voiceTranscription: {
		getConfig: () =>
			ipcRenderer.invoke(
				ipcChannels.voiceTranscriptionGetConfig,
			) as Promise<VoiceTranscriptionPublicConfig>,
		saveConfig: (config: VoiceTranscriptionSaveInput) =>
			ipcRenderer.invoke(
				ipcChannels.voiceTranscriptionSaveConfig,
				config,
			) as Promise<VoiceTranscriptionSaveResult>,
		transcribe: (request: VoiceTranscriptionRequest) =>
			ipcRenderer.invoke(
				ipcChannels.voiceTranscriptionTranscribe,
				request,
			) as Promise<VoiceTranscriptionResult>,
		cancel: (requestId: string) =>
			ipcRenderer.invoke(
				ipcChannels.voiceTranscriptionCancel,
				requestId,
			) as Promise<void>,
	},
	...
	catalog: {
		status: () =>
			ipcRenderer.invoke(
				ipcChannels.catalogUpdateStatus,
			) as Promise<CatalogUpdateStatus>,
		check: (branch: "main" | "dev") =>
			ipcRenderer.invoke(
				ipcChannels.catalogUpdateCheck,
				branch,
			) as Promise<CatalogCheckResult>,
		updateFromGithub: (branch: "main" | "dev") =>
			ipcRenderer.invoke(
				ipcChannels.catalogUpdateFromGithub,
				branch,
			) as Promise<CatalogUpdateResult>,
		restore: () =>
			ipcRenderer.invoke(
				ipcChannels.catalogUpdateRestore,
			) as Promise<CatalogUpdateResult>,
		restorePrevious: () =>
			ipcRenderer.invoke(
				ipcChannels.catalogUpdateRestorePrevious,
			) as Promise<CatalogUpdateResult>,
		/** 用系统默认程序打开当前生效的目录文件（覆盖层优先，否则内置） */
		openFile: () =>
			ipcRenderer.invoke(ipcChannels.catalogOpenFile) as Promise<void>,
	},
	...
	automation: {
		getSnapshot: () =>
			ipcRenderer.invoke(
				ipcChannels.automationGetSnapshot,
		...
		previewCron: (expression: string, count?: number) =>
			ipcRenderer.invoke(
				...
				expression,
				count,
			) as Promise<AutomationCronPreview>,
