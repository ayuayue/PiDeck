...
export function useProjectSync(input: UseProjectSyncInput) {
	const { projects, activeProjectId, setProjects, setActiveProjectId, replaceProjectSessions, api, showToast, setSessionCatalogLoadState, t } = input;
	...
	async function refreshProjects(): Promise<Project[]> {
		...
		// 工作区模式的项目分支随 worktree 列表一起取（refreshWorktrees），供 worktree 主工作区行展示。
		for (const p of next) {
	...
	async function refreshWorktrees(projectId: string) {
		try {
			const [entries, branchInfo] = await Promise.all([api.git.worktreeList(projectId), api.git.branches(projectId).catch(() => ({ current: null, branches: [] }))]);
	...
	async function runProjectSessionRefresh(projectId: string, silent: boolean, completion: ProjectSessionRefreshCompletion): Promise<void> {
		...
		try {
			...
			const records = await withTimeout(api.sessions.listCatalog(projectId), SESSION_REFRESH_TIMEOUT_MS, t("app.sessionRefreshTimeout"));
	...
	function startProjectSessionRefresh(projectId: string, silent: boolean, completion: ProjectSessionRefreshCompletion) {
	...
	async function refreshFiles(projectId = activeProjectId, silent = false, expandedDirs: Iterable<string> = []) {
		if (!projectId) return;
