...
export function ConfirmDialog(props: { title: string; message: string; onConfirm: () => void; onCancel: () => void; confirmLabel?: string; danger?: boolean }) {
	// GUI 扩展桥：对话框是应用级单实例 chrome，按「当前聚焦会话」取桥状态。
	...
	return (
		...
		>
			<AlertDialogContent>
				...
				<BridgeGuiSlot sessionId={bridgeChromeSessionId} slot="dialog.body" className="flex flex-col gap-2" />
				<AlertDialogFooter>
					{/* GUI 扩展桥：对话框按钮区落点（ctx.gui.setDialogAction）。
					    放在既有取消/确认按钮**之前**，不改变原有按钮顺序与语义。 */}
					<BridgeGuiSlot sessionId={bridgeChromeSessionId} slot="dialog.action" className="flex items-center gap-2" />
