...
const workspaceRowClass = "workspace-tree-row relative flex min-h-8 min-w-0 items-center gap-0.5 rounded-md p-0.5 text-body text-foreground transition-[background-color,border-color,box-shadow] duration-fast hover:bg-muted/60";
...
const workspaceActionPaddingClass = "group-hover/workspace-row:pr-[52px] group-focus-within/workspace-row:pr-[52px]";
 ...
 */
function WorkspaceRowActions(props: { children: React.ReactNode; menuOpen?: boolean }) {
	return (
		<div
			className={cn(
		...
		<div className="workspace-tree min-w-0 py-1 pl-1">
			<section className="workspace-tree-main" aria-label={t("app.worktreeMainWorkspace")}>
				<div className={cn(workspaceRowClass, "group/workspace-row")}>
					...
					>
						<span className="worktree-main-branch-icon grid size-5 shrink-0 place-items-center text-muted-foreground">
							<GitBranch size={14} />
						</span>
						<span className={cn("conversation-body min-w-0 flex-1 transition-[padding-right]", workspaceActionPaddingClass, mainActionsOpen && "pr-[52px]")}>
							<span className="conversation-title flex min-w-0 items-center gap-1.5">
								...
								{mainPendingAskCount > 0 && (
									<Badge variant="outline" className="h-4 shrink-0 gap-0.5 border-amber-500/40 bg-amber-500/15 px-1 py-0 text-[10px] font-medium leading-none text-amber-600 dark:text-amber-400" title={t("sidebar.pendingConfirmationHint", { count: String(mainPendingAskCount) })}>
					...
					<WorkspaceRowActions menuOpen={mainActionsOpen}>
						<Button type="button" variant="ghost" size="icon-xs" className={workspaceActionClass} aria-label={t("app.newNormalSession")} title={t("app.newNormalSession")} onClick={() => void props.actions.sessions.createDraft(props.project.id)}>
            ...
            不能等 Worktree 列表渲染完再由 ProjectTree 追加到所有工作区之后。 */}
				{!mainCollapsed && (
					<div id={mainRowId} className={cn("workspace-tree-main-sessions", workspaceSessionsClass)}>
						<SessionTree project={props.project} sessions={props.sessions} agents={props.agents} currentSessionId={props.currentSessionId} controller={props.controller} actions={props.actions} />
					</div>
				)}
			</section>
			...
			<section className="workspace-tree-list" aria-label={t("app.worktreeOtherWorkspaces")}>
				<header className="workspace-tree-section-header mt-2 flex min-h-7 items-center justify-between gap-2 border-t border-border/40 px-2 pt-1 text-micro text-muted-foreground">
					...
					<Button type="button" variant="ghost" size="icon-xs" className="workspace-tree-create" title={t("app.worktreeNew")} aria-label={t("app.worktreeNew")} onClick={() => props.controller.openWorktreeCreate(props.project.id)}>
						<Plus size={13} />
					</Button>
				</header>
			...
			<div className="workspace-tree-header group/workspace-row relative flex min-w-0 flex-1 items-center gap-0.5">
				...
				>
					...
					{childPendingAskCount > 0 && (
						<Badge variant="outline" className="h-4 shrink-0 gap-0.5 border-amber-500/40 bg-amber-500/15 px-1 py-0 text-[10px] font-medium leading-none text-amber-600 dark:text-amber-400" title={t("sidebar.pendingConfirmationHint", { count: String(childPendingAskCount) })}>
							<HelpCircle className="size-2.5 shrink-0 animate-pulse" aria-hidden="true" />
					...
					{row.directory !== row.branch && <span className="workspace-tree-directory max-w-20 shrink-0 truncate text-micro text-muted-foreground">{row.directory}</span>}
				</button>

				{childProject && (
					<WorkspaceRowActions menuOpen={childActionsOpen}>
						<Button type="button" variant="ghost" size="icon-xs" className={workspaceActionClass} aria-label={t("app.newNormalSession")} title={t("app.newNormalSession")} onClick={() => void props.actions.sessions.createDraft(childProject.id)}>
							<Plus size={13} aria-hidden="true" />
						</Button>
						<Button
							type="button"
