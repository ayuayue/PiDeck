 ...
 */
export const SessionRuntimeInjector = React.memo(
  function SessionRuntimeInjector(props: SessionRuntimeInjectorProps) {
    const {
      currentSessionId,
      sessionTitle,
      sessionTimeline,
    ...
    const currentSessionRuntimeUi = useAtomValue(
      sessionRuntimeUiBySessionIdAtomFamily(currentSessionId),
    );
    const claimSessionUiResponse = useSetAtom(
      claimSessionRuntimeUiResponseAtom,
    );
    const rollbackSessionUiResponse = useSetAtom(
      rollbackSessionRuntimeUiResponseAtom,
    );
    ...
    const paneProjectId =
      currentSessionRuntime?.projectId ?? sessionRecord?.projectId ?? "";
    const paneProject = useAtomValue(projectByIdAtomFamily(paneProjectId));
    ...
    const paneOwnerKey = paneTerminal
      ? terminalOwnerKey(paneTerminal.owner)
      : undefined;
    const paneTerminalState = paneOwnerKey
      ? services.terminalStatesByOwner[paneOwnerKey]
      : undefined;
    const paneTerminalOpen =
      Boolean(paneTerminalState?.open) && Boolean(paneTerminal);
    ...
    const setPaneTerminalOpen = React.useCallback(
      (open: boolean) => {
        if (paneOwnerKey)
          services.setTerminalOpenByOwnerKey(paneOwnerKey, open);
      },
    ...
    const setPaneTerminalCollapsed = React.useCallback(
      (collapsed: boolean) => {
        if (paneOwnerKey)
          services.setTerminalCollapsedByOwnerKey(paneOwnerKey, collapsed);
      },
    ...
    const isDshBackend =
      sessionRecord?.backend === "dsh" || activeAgent?.backend === "dsh";
    ...
    const [rewindConfirm, setRewindConfirm] =
      React.useState<RewindCheckpointSummary | null>(null);
    ...
    const handleRewindToMessage = React.useCallback(
      async (message: ChatMessage) => {
        ...
        try {
          ...
          services.showToast(
            sessionCommandFailureToast(error, (raw) =>
              t("rewind.loadFailed", { error: raw }),
            ),
            4000,
          );
          return;
    ...
    const performRewindRestore = React.useCallback(async () => {
      ...
      try {
        await requireSessionCommand(
          await desktopApi.sessions.restoreRewindCheckpoint(
            target,
            rewindConfirm.id,
            "files",
          ),
        ...
        services.showToast(
          sessionCommandFailureToast(error, (raw) =>
            t("rewind.restoreFailed", { error: raw }),
          ),
          4000,
        );
      } finally {
      ...
      >
        <>
          <SessionView
            ...
            onResendUserMessage={
              canResend ? services.resendUserMessage : undefined
            }
            onEditMessage={
              canEditOrDeleteMessages ? services.editMessage : undefined
            }
            onDeleteMessage={
              canEditOrDeleteMessages ? services.deleteMessage : undefined
            }
            ...
            onQuickPrompt={(message) =>
              services.insertQuickPrompt(currentSessionId, message)
            }
            canMutateActiveMessages={canMutateActiveMessages}
            onOpenBranchSession={
              services.activeProjectId && services.openSidebarSessionById
                ? (sessionId: string) => {
                    void services.openSidebarSessionById?.(
                      services.activeProjectId!,
                      sessionId,
                    );
                  }
                : undefined
