...
function pruneRuntimeNotificationKeys(): void {
 ...
 for (const retainedKey of retained)
  seenRuntimeNotificationKeys.add(retainedKey);
}
...
export function describeBackgroundAsk(input: {
 ...
 const question = input.requestTitle
  ? formatAskTitle(input.requestTitle) || undefined
  : undefined;
