...
export const enUS: Record<TranslationKey, string> = {
	...
	"app.modelClearSelection": "Clear selection",
	"app.modelClearSelectionHint": "Clear the welcome or unstarted session selection and return to default resolution; running models are unchanged.",
	...
	"commitSuggest.commitPush": "Commit & push",
	"commitSuggest.commitPushText": "Please commit the current changes and push to the remote, with a message following repository conventions.",
	...
	"ask.batchTitle": "Questions ({count})",
	...
	"command.openHiddenSettingsHint": "Hidden — click to show",
	...
	// Toast history panel (opened from Settings → Notifications or the details dialog)
	"notice.historyTitle": "Notification history",
	"notice.historyDesc": "All toasts shown during this run (kept in memory only, cleared on restart). Copy or re-show any entry.",
	"notice.historyEmpty": "No notifications yet",
	"notice.historyClear": "Clear history",
	"notice.historyReplay": "Show again",
	"notice.historySticky": "sticky",
	"notice.historyOpen": "View all notifications",
	// Toast stack pill: shown bottom-right when active toasts exceed the visible stack
	"notice.stackOverflow": "{count} more notifications hidden",
	"notice.stackOverflowTitle": "Open notification history",
	// History table: kind filter + keyword search + pagination
	"notice.historySearchPlaceholder": "Search title or detail",
	"notice.historyKindFilter": "Filter by level",
	"notice.historyKindAll": "All levels",
	"notice.historyKind.neutral": "Plain",
	"notice.historyKind.info": "Info",
	"notice.historyKind.warning": "Warning",
	"notice.historyKind.error": "Error",
	"notice.historyKind.question": "Waiting",
	"notice.historyColumn.time": "Time",
	"notice.historyColumn.kind": "Level",
	"notice.historyColumn.title": "Title",
	"notice.historyColumn.detail": "Detail",
	"notice.historyColumn.duration": "Duration",
	"notice.historyColumn.actions": "Actions",
	"notice.historyResults": "{count} entries",
	"notice.historyNoMatch": "No matching notifications — try another keyword or level",
	...
	"qoder.emptyDesc": "Only scans sessions in `~/.qoder-cn/projects` where cwd matches the current project.",
	"qoder.emptyTitle": "No Qoder sessions found for this project",
	"qoder.importCount": "{count} importable sessions",
	"qoder.importDone": "Import complete: {imported} succeeded, {failed} failed",
	"qoder.importFailed": "Failed to import Qoder sessions: {error}",
	"qoder.importSelected": "Import {count}",
	"qoder.importing": "Importing...",
	"qoder.scanFailed": "Failed to scan Qoder sessions: {error}",
	"qoder.scanning": "Scanning Qoder sessions for this project...",
	"qoder.selectNone": "Deselect All",
	"qoder.status.current": "Current",
	"qoder.status.new": "Not imported",
	"qoder.status.outdated": "Can update",
	"qoder.title": "Import Qoder Sessions",
	...
	"config.dsh.baseUrlHint": "DSH uses this URL according to its adapter, without the Pi probe's / and /v1 fallback. Enter the API base URL provided by the service.",
	"config.dsh.addProviderHint": "Enter connection details, then fetch or add models and click Add or the top Save button. This uses DSH discovery and validation, not Pi probes or protocol patches.",
	"config.dsh.providerCredentialHint": "DSH stores the key separately in credentials; settings.yaml only stores the reference {ref}. Names such as Chinese automatically receive a safe reference and do not restrict Pi provider names.",
	"config.dsh.catalogProviderHint": "Built-in providers inherit the DSH adapter protocol and model catalog. You may override only the endpoint or key; an empty model list inherits the catalog.",
	"config.dsh.customProviderHint": "Custom providers need a Base URL, API type and at least one valid model. DSH adapters resolve model capabilities; Pi-only compatibility fields are not added.",
	"config.dsh.addProviderIncomplete": "Check the provider name for invalid or duplicate entries. Custom providers also need a Base URL, API type and valid models with unique IDs and positive integer capacities.",
	...
	"config.usageProbe.category.volcengine": "Volcengine Ark AK/SK",
	...
	"config.usageProbe.volcengineAccessKeyId": "Access Key ID (AK)",
	"config.usageProbe.volcengineAccessKeyIdPlaceholder": "API Key ID from the Volcengine console (Access Control → Keys), e.g. AKLT...",
	"config.usageProbe.volcengineAccessKeyIdRequired": "Enter the Access Key ID (console → Access Control → Keys)",
	"config.usageProbe.volcengineSecretKey": "Secret Access Key (SK)",
	"config.usageProbe.volcengineSecretKeyPlaceholder": "The paired secret; used only for local signing — never uploaded or logged",
	"config.usageProbe.volcengineSecretKeyRequired": "Enter the Secret Access Key (generated together with the AK)",
	...
	// Model config export/import (Base64 transfer)
	"config.models.transfer.exportSelected": "Export selected ({count})",
	"config.models.transfer.importButton": "Import",
	"config.models.transfer.exportTitle": "Export model config",
	"config.models.transfer.importTitle": "Import model config",
	"config.models.transfer.selectedProviders": "Will export {count} providers (API keys included)",
	"config.models.transfer.password": "Encryption password (optional)",
	"config.models.transfer.passwordHint": "Leave empty for no encryption; importing an encrypted export requires the same password",
	"config.models.transfer.noEncryptHint": "An unencrypted export is only encoded, not protected — anyone with the string can read everything, including API keys",
	"config.models.transfer.exportButton": "Generate export",
	"config.models.transfer.copyBase64": "Copy Base64",
	"config.models.transfer.saveFile": "Save to file",
	"config.models.transfer.copied": "Copied to clipboard",
	"config.models.transfer.saved": "File saved",
	"config.models.transfer.pasteLabel": "Paste Base64 config",
	"config.models.transfer.pastePlaceholder": "Paste base64 string…",
	"config.models.transfer.pickFile": "Choose file",
	"config.models.transfer.parse": "Parse",
	"config.models.transfer.invalid": "Cannot parse: invalid data format",
	"config.models.transfer.unsupportedVersion": "Cannot parse: unsupported version",
	"config.models.transfer.wrongKind": "Cannot parse: not model config data",
	"config.models.transfer.needPassword": "This export is encrypted; enter the password",
	"config.models.transfer.wrongPassword": "Wrong password or corrupted data",
	"config.models.transfer.selectProviders": "Select providers to import",
	"config.models.transfer.conflictWithLocal": "Conflicts with local",
	"config.models.transfer.newProvider": "New",
	"config.models.transfer.next": "Next",
	"config.models.transfer.overwrite": "Overwrite",
	"config.models.transfer.merge": "Merge",
	"config.models.transfer.overwriteAll": "Overwrite all",
	"config.models.transfer.mergeAll": "Merge all",
	"config.models.transfer.keepAllLocal": "Keep all local",
	"config.models.transfer.adoptAllImported": "Adopt all imported",
	"config.models.transfer.local": "Local",
	"config.models.transfer.imported": "Imported",
	"config.models.transfer.providerFields": "Provider config differences",
	"config.models.transfer.modelFields": "Model config differences",
	"config.models.transfer.newModels": "New models (auto-merged): {ids}",
	"config.models.transfer.apply": "Apply import",
	"config.models.transfer.applied": "Applied as unsaved changes — review and click Save",
	"config.models.transfer.done": "Done",
	"config.models.transfer.noProvidersSelected": "Select at least one provider",
	...
	"menu.importQoder": "Import Qoder Sessions",
	...
	"settings.shortcuts.toggleVoiceRecordingLabel": "Start/Stop Voice Input",
	"settings.shortcuts.toggleVoiceRecordingDesc": "Toggles recording in the focused session; works once voice input is enabled in settings. Default F9.",
	"settings.shortcuts.zoomInLabel": "Zoom In",
	"settings.shortcuts.zoomInDesc": "Zooms the whole window in (icons and spacing included). Default Ctrl+= (⌘= on macOS).",
	"settings.shortcuts.zoomOutLabel": "Zoom Out",
	"settings.shortcuts.zoomOutDesc": "Zooms the whole window out (icons and spacing included). Default Ctrl+- (⌘- on macOS).",
	...
	"settings.navigationTabs": "Tabs",
	"settings.navigationSimple": "Simple mode",
	"settings.navigationModeDesc": "Use one sidebar for sessions and a right-hand area for files and diffs. Applies after saving.",
	...
	"settings.startupWindow.last": "Last window position and size",
	...
	"settings.modules.sectionDesc": "Turn off a module you don't use to hide its settings page and entry points. Only the UI is hidden — configuration is kept and enabled features keep running. Hidden pages can still be found via the command palette (Ctrl+P).",
	"settings.modules.dsh": "DSH backend",
	"settings.modules.dshDesc": "DSH tab in Configuration and the DSH backend option in the composer",
	"settings.modules.imDesc": "Feishu Bot settings page",
	"settings.modules.petDesc": "Desktop Pet settings page",
	"settings.modules.visionDesc": "Vision Bridge settings page",
	"settings.modules.imagegenDesc": "Image Gen settings page and the image generation entry in the composer",
	"settings.modules.webDesc": "LAN Web Service settings page",
	"settings.modules.gitDesc": "Git settings page (the Git panel entry is controlled by the switch inside that page)",
	"settings.modules.usageDesc": "Usage Stats page",
	"settings.modules.processDesc": "Process Monitor page",
	"settings.modules.statusConnected": "Connected",
	"settings.modules.statusRunning": "Session running",
	"settings.modules.statusEnabled": "Enabled",
	"settings.modules.statusConfigured": "Configured",
	"settings.modules.revealedNotice": '"{tab}" is hidden and shown temporarily; restore it under Appearance → Feature Modules',
	...
	"settings.toastHistoryDesc": "Review all toasts shown during this run (kept in memory, cleared on restart)",
	"settings.toastHistoryOpen": "View history",
	...
	"settings.quickMessagesSectionDesc": "Quick messages use your personal config file. Built-in updates are not merged automatically; add missing defaults from the management dialog.",
	...
	"settings.quickMessagesDesc": "Drag the left handle or use the up/down buttons to reorder — keep the most-used on top. Up to {max} items; blank rows are not written to the config file.",
	"settings.quickMessagesConfigHint": "Your personal list uses quick-messages.json; built-in defaults come from resources/quick-messages.default.json and are not merged automatically. “Add missing defaults” appends missing items, keeping your content and order; “Restore defaults” replaces the entire list.",
	"settings.quickMessagesEmpty": "No quick messages yet: add one, or use “Add missing defaults” to import the built-in list.",
	"settings.quickMessagesPlaceholder": "e.g. continue",
	"settings.quickMessagesAdd": "Add",
	"settings.quickMessagesMergeDefaults": "Add missing defaults",
	"settings.quickMessagesMergeDefaultsHint": "Reload built-in defaults and append only missing items, keeping your current order (subject to the item limit)",
	"settings.quickMessagesMerging": "Adding defaults…",
	"settings.quickMessagesNothingToAdd": "Your list already includes all built-in items",
	"settings.quickMessagesReset": "Restore defaults",
	"settings.quickMessagesResetHint": "Replace your entire list with built-in defaults (custom content and order will be lost)",
	...
	"settings.quickMessagesReloadHint": "Re-read your personal config file after editing it by hand; this does not add missing defaults",
	"settings.quickMessagesFileHint": "Personal config file: {path}",
	...
	"settings.quickMessagesDragHandle": "Drag to reorder (or use the up/down buttons)",
	...
	// Process group header copy (contract §2): category labels + join fragments, for both running and done states.
	...
	"voice.finalizing": "Finishing last segment",
	...
	"voice.error.configReadFailed": "Could not read voice transcription settings. Please try again.",
	"voice.error.blocked.disabled": "Voice input is off. Enable it in Common settings.",
	"voice.error.blocked.cloudMissingKey": "Cloud transcription needs an API key. Add one in Common settings.",
	"voice.error.blocked.cloudMissingEndpoint": "Cloud transcription needs a base URL and model. Complete them in Common settings.",
	"voice.error.blocked.localRuntime": "Local whisper is not ready (missing CLI or model). Install it in Common settings.",
	"voice.error.blocked.unknown": "Configure voice transcription in Common settings first",
	...
	"voice.error.empty": "No speech was recognized",
	"voice.error.noSpeech": "No audio was recorded — move closer to the microphone and try again",
	"voice.error.staleTarget": "The input changed, so the transcription was not inserted",
	"voice.settings.title": "Voice input",
	"voice.settings.description": "Choose cloud (OpenAI-compatible) or local whisper.cpp transcription. The microphone button appears only when enabled; local models download on demand and never grow the installer.",
	"voice.settings.enabled": "Enable voice input",
	"voice.settings.enabledDescription": "Hides the microphone button in the composer when off.",
	"voice.settings.engine": "Transcription engine",
	"voice.settings.engineCloud": "Cloud (online transcription service)",
	"voice.settings.engineLocal": "Local (whisper.cpp, offline)",
	"voice.settings.device": "Recording device",
	"voice.settings.deviceDescription": "Pick a microphone; device names appear after you grant access.",
	"voice.settings.deviceDefault": "System default",
	"voice.settings.runtime": "Local runtime (whisper-cli)",
	"voice.settings.runtimeReady": "Ready — transcription works offline.",
	"voice.settings.runtimeMissing": "Not downloaded yet — local transcription unavailable.",
	"voice.settings.runtimeSourceAuto": "Auto-downloaded",
	...
	"voice.settings.modelDescription": "Larger models are more accurate but slower and heavier (tiers below 181MB have been retired). Select one, then download it here.",
	...
	"voice.settings.modelDelete": "Delete model",
	"voice.settings.cliPath": "whisper-cli path",
	"voice.settings.cliPathDescription": "Optional: point at a whisper-cli executable when auto-download is unavailable.",
	"voice.settings.cliPathPlaceholder": "e.g. …/whisper-cli.exe or /usr/local/bin/whisper-cli",
	"voice.settings.phase.verifying": "Verifying",
	"voice.settings.phase.installing": "Installing",
	"voice.settings.phase.done": "Done",
	"voice.settings.phase.error": "Failed",
	"voice.settings.installCancel": "Cancel download",
	"voice.settings.modelPartial": "{size} already downloaded — clicking download again resumes from that point",
	"voice.settings.autoSaveHint": "Changes are saved automatically",
	"voice.settings.apiKeyAutoSaveHint": "The key is saved when the field loses focus",
	"voice.settings.error.installFailed": "Failed to download or install the local voice component",
	"voice.settings.error.cancelled": "Download cancelled",
	"voice.settings.error.alreadyInstalling": "A download is already running. Wait for it or cancel it first.",
	"voice.error.engineUnavailable": "Local voice engine is not ready. Download the runtime and model in settings first.",
	...
	"voice.settings.cloudProvider": "Cloud provider",
	"voice.settings.cloudProviderDescription": "OpenAI-compatible uses /audio/transcriptions; Doubao Speech uses the big-model file-recognition flash API, which you must first enable under Activation management at console.volcengine.com/speech (new apps get 20 free hours).",
	"voice.settings.cloudProviderOpenai": "OpenAI-compatible",
	"voice.settings.cloudProviderVolc": "Volcengine · Doubao Speech",
	"voice.settings.volcAppId": "App ID",
	"voice.settings.volcAppIdDescription": "The App ID from the console's app management (legacy apps). The new console issues a single API key — put it here and leave the access token blank.",
	"voice.settings.volcAccessToken": "Access token",
	"voice.settings.volcAccessTokenDescription": "Mandatory next to the App ID on the legacy console — one without the other always fails auth; leave blank on the new console.",
	"voice.settings.cloudResourceId": "Resource ID",
	"voice.settings.cloudResourceIdDescription": "It decides which protocol the server parses the request with, so only capabilities this client implements are listed.",
	"voice.settings.cloudResourceIdTurbo": "Big-model file recognition · flash",
	"voice.settings.test": "Test connection",
	"voice.settings.test.ok": "Link is fine: the service accepted the probe audio, you can start recording",
	"voice.settings.test.unexpected": "The probe request could not complete. Try again.",
	"voice.settings.test.volcHint": "For Doubao Speech, check that this resource is activated and the quota is not exhausted (quote this code in a support ticket).",
	"voice.error.blocked.cloudMissingVolcAppId": "Doubao Speech needs an App ID. Add one in Common settings.",
	"voice.settings.model": "Model",
	"voice.settings.language": "Language",
	"voice.settings.languageDescription": "Optional ISO-639-1 language code; blank means auto-detect. Doubao speech maps it to a regional tag such as zh-CN.",
