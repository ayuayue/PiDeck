...
export const zhCN = {
	...
	"config.models.batchActionLabel": "请选择操作",
	"config.models.batchAction.delete": "批量删除",
	"config.models.batchAction.export": "批量导出",
	"config.models.batchExecute": "批量执行",
	...
	"config.models.transfer.exportToClipboard": "导出到剪贴板",
	"config.models.transfer.exportToFile": "导出到文件",
	...
	// 更新通道（徽章 + 切换向导，规格 §5）：确认区文案含数据模式预告
	"settings.channelBadgeStable": "Stable",
	"settings.channelBadgeDev": "Dev",
	"settings.channelSwitchToDev": "切换到开发版",
	"settings.channelSwitchToStable": "切换回稳定版",
	"settings.channelSwitchDialogTitle": "切换更新通道",
	"settings.channelSwitchQuerying": "正在查询目标通道最新版本…",
	"settings.channelSwitchTargetVersion": "目标版本：v{version}",
	"settings.channelSwitchNotes": "更新说明",
	"settings.channelSwitchDataModeNotice": "安装后首次启动会让你选择数据模式",
	"settings.channelSwitchDownload": "下载",
	"settings.channelSwitchInstallNow": "立即安装",
	"settings.channelSwitchOpenReleasePage": "打开发布页",
	"settings.channelSwitchError": "切换失败：{error}",
	// 数据环境（dev 首启数据模式选择 + 导入向导 + 目录标记警告，规格 §5/§6）；选项文案逐字来自规格 §5
	"dataMode.dialogTitle": "选择数据模式",
	"dataMode.dialogDesc": "请选择开发版的数据目录方式；之后可在设置的「开发设置」中更改。",
	"dataMode.sharedTitle": "与正式版共用",
	"dataMode.sharedDesc": "与正式版共用同一数据目录，可无缝来回切换通道；dev 写入的新设置可能暂不被正式版识别",
	"dataMode.channelDevTitle": "独立数据目录",
	"dataMode.channelDevDesc": "使用独立数据目录，初次可从正式版导入快照；此后在 dev 中产生的会话、设置等变更不会自动同步回正式版，需要自行处理带回",
	"dataMode.importSnapshot": "从正式版导入数据",
	"dataMode.importSnapshotDesc": "导入在重启前完成，期间请勿关闭应用。",
	"dataMode.importEstimating": "正在估算迁移数据体积…",
	"dataMode.importPreviewSize": "预计迁移数据：{size}",
	"dataMode.importStart": "开始导入",
	"dataMode.importRunning": "正在导入…",
	"dataMode.importCopying": "正在导入：{item}",
	"dataMode.importDone": "导入完成，应用即将重启…",
	"dataMode.importCancelled": "已取消导入。已复制的内容已保留，可再次导入。",
	"dataMode.importSkip": "跳过导入",
	"dataMode.importUnavailable": "当前环境无法导入数据",
	"dataMode.importError": "导入失败，请重试",
	"dataMode.modeChangeFailed": "操作失败，请重试",
	"dataMode.mismatchTitle": "数据目录与当前通道不匹配",
	"dataMode.mismatchDesc": "当前数据目录属于另一通道的数据模式，继续使用可能导致两个通道的数据互相混淆。",
	"dataMode.mismatchContinue": "继续使用",
	"dataMode.mismatchQuit": "退出",
	"settings.updateSourceDevLocked": "开发版固定使用 GitHub 更新源（预发布通道）",
	"settings.dataEnvSectionTitle": "数据环境",
	"settings.dataEnvCurrentMode": "当前数据模式",
	"settings.dataEnvSwitchButton": "切换数据模式",
	"settings.dataEnvSwitchConfirmDesc": "修改需重启应用后生效",
