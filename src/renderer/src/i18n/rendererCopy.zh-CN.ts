...
export const zhCN = {
	...
	"app.modelClearSelection": "清空预选",
	"app.modelClearSelectionHint": "清除当前引导页或未启动会话的模型预选，恢复默认解析；不修改运行中的模型",
	...
	"commitSuggest.commitPush": "提交并推送",
	"commitSuggest.commitPushText": "请提交当前改动并推送到远程：按仓库惯例写好提交信息。",
	...
	"command.openHiddenSettingsHint": "已隐藏，点击显示",
	...
	// toast 通知历史面板（设置页「通知」区与详情弹窗共用同一入口打开）
	"notice.historyTitle": "通知历史",
	"notice.historyDesc": "本次运行期间弹出的全部通知（仅存内存，重启后清空）。可复制全文或重新弹出。",
	"notice.historyEmpty": "暂无通知记录",
	"notice.historyClear": "清空记录",
	"notice.historyReplay": "重新弹出",
	"notice.historySticky": "常驻",
	"notice.historyOpen": "查看全部通知",
	// toast 堆叠收纳条：活跃 toast 超过可见堆叠数时出现在右下角
	"notice.stackOverflow": "还有 {count} 条通知未展示",
	"notice.stackOverflowTitle": "打开通知历史查看全部",
	// 历史表格：级别筛选 + 关键词搜索 + 分页
	"notice.historySearchPlaceholder": "搜索标题或详情",
	"notice.historyKindFilter": "按级别筛选",
	"notice.historyKindAll": "全部级别",
	"notice.historyKind.neutral": "普通",
	"notice.historyKind.info": "信息",
	"notice.historyKind.warning": "警告",
	"notice.historyKind.error": "错误",
	"notice.historyKind.question": "待回答",
	"notice.historyColumn.time": "时间",
	"notice.historyColumn.kind": "级别",
	"notice.historyColumn.title": "标题",
	"notice.historyColumn.detail": "详情",
	"notice.historyColumn.duration": "时长",
	"notice.historyColumn.actions": "操作",
	"notice.historyResults": "共 {count} 条",
	"notice.historyNoMatch": "没有匹配的通知，换个关键词或级别试试",
	"common.delete": "删除",
	"common.paste": "粘贴",
	...
	"qoder.emptyDesc": "只会扫描 `~/.qoder-cn/projects` 中 cwd 与当前项目一致的会话。",
	"qoder.emptyTitle": "没有找到当前项目的 Qoder 会话",
	"qoder.importCount": "{count} 个可导入会话",
	"qoder.importDone": "导入完成：{imported} 成功，{failed} 失败",
	"qoder.importFailed": "导入 Qoder 会话失败：{error}",
	"qoder.importSelected": "导入 {count}",
	"qoder.importing": "导入中...",
	"qoder.scanFailed": "扫描 Qoder 会话失败：{error}",
	"qoder.scanning": "正在扫描当前项目的 Qoder 会话...",
	"qoder.selectNone": "取消全选",
	"qoder.status.current": "已是最新",
	"qoder.status.new": "未导入",
	"qoder.status.outdated": "可覆盖更新",
	"qoder.title": "导入 Qoder 会话",
	...
	"config.dsh.baseUrlHint": "DSH 按适配器规则使用此地址，不执行 Pi 探针的 / 与 /v1 回退；请填写服务方提供的 API 根地址。",
	"config.dsh.addProviderHint": "先填写连接信息并获取或手动添加模型，再点击「添加」或顶部「保存」。这里使用 DSH 的模型发现和配置校验，不复用 Pi 的探针或协议补丁。",
	"config.dsh.providerCredentialHint": "DSH 将密钥单独存入 credentials，settings.yaml 仅保存引用 {ref}。中文等名称会自动生成安全引用，无需改成英文；也不影响 Pi 供应商命名。",
	"config.dsh.catalogProviderHint": "内置提供方沿用 DSH 适配器的协议和模型目录，可仅覆盖地址或密钥；留空模型列表表示继承目录。",
	"config.dsh.customProviderHint": "自定义提供方需填写 Base URL、API 类型及至少一个有效模型。DSH 模型能力由适配器解析，不会自动写入 Pi 专属的兼容性字段。",
	"config.dsh.addProviderIncomplete": "请检查名称是否有效或重复，并为自定义提供方填写 Base URL、API 类型和有效模型（ID 不可重复，容量须为正整数）。",
	...
	"config.usageProbe.category.volcengine": "火山方舟 AK/SK",
	...
	"config.usageProbe.volcengineHint": "火山方舟（ark）套餐用量：用控制台生成的 AK/SK 签名查询 Agent Plan 或 Coding Plan 额度，自动探测账号订购的套餐",
	...
	"config.usageProbe.volcengineAccessKeyIdPlaceholder": "火山引擎控制台「访问控制 → 密钥管理」里的 API Key ID，如 AKLT...",
	"config.usageProbe.volcengineAccessKeyIdRequired": "请填写 Access Key ID（控制台「访问控制 → 密钥管理」）",
	"config.usageProbe.volcengineSecretKey": "Secret Access Key（SK）",
	"config.usageProbe.volcengineSecretKeyPlaceholder": "与 AK 配对生成的密钥，仅用于本地签名，不会上传或记录日志",
	"config.usageProbe.volcengineSecretKeyRequired": "请填写 Secret Access Key（与 AK 一同生成）",
	...
	// 模型配置导出/导入（Base64 传输）
	"config.models.transfer.exportSelected": "导出选中 ({count})",
	"config.models.transfer.importButton": "导入",
	"config.models.transfer.exportTitle": "导出模型配置",
	"config.models.transfer.importTitle": "导入模型配置",
	"config.models.transfer.selectedProviders": "将导出 {count} 个供应商（含 API Key）",
	"config.models.transfer.password": "加密密码（可选）",
	"config.models.transfer.passwordHint": "留空则不加密；加密后导入时需输入同一密码",
	"config.models.transfer.noEncryptHint": "未加密的导出仅作编码、不构成保护，任何拿到字符串的人都能读出全部内容（含 API Key）",
	"config.models.transfer.exportButton": "生成导出",
	"config.models.transfer.copyBase64": "复制 Base64",
	"config.models.transfer.saveFile": "保存为文件",
	"config.models.transfer.copied": "已复制到剪贴板",
	"config.models.transfer.saved": "文件已保存",
	"config.models.transfer.pasteLabel": "粘贴 Base64 配置",
	"config.models.transfer.pastePlaceholder": "粘贴 base64 字符串…",
	"config.models.transfer.pickFile": "选择文件",
	"config.models.transfer.parse": "解析",
	"config.models.transfer.invalid": "无法解析：数据格式不正确",
	"config.models.transfer.unsupportedVersion": "无法解析：不支持的版本",
	"config.models.transfer.wrongKind": "无法解析：不是模型配置数据",
	"config.models.transfer.needPassword": "该导出已加密，请输入密码",
	"config.models.transfer.wrongPassword": "密码错误或数据损坏",
	"config.models.transfer.selectProviders": "选择要导入的供应商",
	"config.models.transfer.conflictWithLocal": "与本地同名",
	"config.models.transfer.newProvider": "新增",
	"config.models.transfer.next": "下一步",
	"config.models.transfer.overwrite": "覆盖",
	"config.models.transfer.merge": "合并",
	"config.models.transfer.overwriteAll": "全部覆盖",
	"config.models.transfer.mergeAll": "全部合并",
	"config.models.transfer.keepAllLocal": "全部保留本地",
	"config.models.transfer.adoptAllImported": "全部采用导入",
	"config.models.transfer.local": "本地",
	"config.models.transfer.imported": "导入",
	"config.models.transfer.providerFields": "供应商配置差异",
	"config.models.transfer.modelFields": "模型配置差异",
	"config.models.transfer.newModels": "新增模型（自动并入）：{ids}",
	"config.models.transfer.apply": "应用导入",
	"config.models.transfer.applied": "已写入未保存更改，请检查后点击「保存」",
	"config.models.transfer.done": "完成",
	"config.models.transfer.noProvidersSelected": "请至少勾选一个供应商",
	...
	"settings.shortcuts.toggleVoiceRecordingLabel": "开始/停止语音输入",
	"settings.shortcuts.toggleVoiceRecordingDesc": "在聚焦的会话中切换录音；开启语音输入后才会生效。默认 F9。",
	"settings.shortcuts.zoomInLabel": "放大窗口",
	"settings.shortcuts.zoomInDesc": "放大整个窗口界面（含图标与间距）。默认 Ctrl+=（macOS ⌘=）。",
	"settings.shortcuts.zoomOutLabel": "缩小窗口",
	"settings.shortcuts.zoomOutDesc": "缩小整个窗口界面（含图标与间距）。默认 Ctrl+-（macOS ⌘-）。",
	...
	"settings.navigationTabs": "标签模式",
	"settings.navigationSimple": "简洁模式",
	"settings.navigationModeDesc": "简洁模式将会话放在左侧单列表，文件与差异放在右侧；保存后生效。",
	"navigation.back": "后退",
	...
	"settings.modules.sectionDesc": "关掉不用的模块可收起它的设置页与入口；只隐藏界面，不清除配置、不停止已启用的功能。命令面板（Ctrl+P）仍可搜到已隐藏的页面。",
	"settings.modules.dsh": "DSH 后端",
	"settings.modules.dshDesc": "配置管理的 DSH 分页、输入框的 DSH 后端选项",
	"settings.modules.imDesc": "飞书机器人设置页",
	"settings.modules.petDesc": "桌面宠物设置页",
	"settings.modules.visionDesc": "视觉桥设置页",
	"settings.modules.imagegenDesc": "生图设置页、输入框的生图入口",
	"settings.modules.webDesc": "局域网 Web 服务设置页",
	"settings.modules.gitDesc": "Git 设置页（Git 面板入口由该页内的开关控制）",
	"settings.modules.usageDesc": "用量统计页",
	"settings.modules.processDesc": "进程监控页",
	"settings.modules.statusConnected": "已连接",
	"settings.modules.statusRunning": "有会话运行中",
	"settings.modules.statusEnabled": "已启用",
	"settings.modules.statusConfigured": "已配置",
	"settings.modules.revealedNotice": "「{tab}」已被隐藏，本次临时显示；可在外观设置→功能模块中恢复",
	...
	"settings.toastHistoryDesc": "查看本次运行期间弹出的全部通知（内存记录，重启后清空）",
	"settings.toastHistoryOpen": "查看历史",
	...
	"settings.quickMessagesSectionDesc": "底栏「快捷消息」使用个人配置文件；内置清单更新不会自动合并，可在管理弹框中补充缺项。",
	...
	"settings.quickMessagesDesc": "拖动左侧把柄或用上下移按钮排序，常用的放前面；最多 {max} 条，空行不会写入配置文件。",
	"settings.quickMessagesConfigHint": "当前使用个人清单 quick-messages.json；内置清单来自 resources/quick-messages.default.json，更新后不会自动合并。「补充内置条目」只在末尾追加缺项，保留个人内容和顺序；「恢复默认」会整体替换。",
	"settings.quickMessagesEmpty": "还没有快捷消息：点「添加」新增，或「补充内置条目」加入内置清单。",
	"settings.quickMessagesPlaceholder": "例如：继续",
	"settings.quickMessagesAdd": "添加",
	"settings.quickMessagesMergeDefaults": "补充内置条目",
	"settings.quickMessagesMergeDefaultsHint": "重新读取内置清单，只追加个人清单缺少的条目，不改变已有顺序（受条数上限约束）",
	"settings.quickMessagesMerging": "正在补充…",
	"settings.quickMessagesNothingToAdd": "个人清单已包含所有内置条目，无需补充",
	"settings.quickMessagesReset": "恢复默认",
	"settings.quickMessagesResetHint": "用内置清单整体替换个人清单（不保留自定义内容和顺序）",
	...
	"settings.quickMessagesReloadHint": "手工编辑个人配置文件后，点这里重新读取；不会自动补充内置条目",
	"settings.quickMessagesFileHint": "个人配置文件：{path}",
	"settings.quickMessagesDefaultsUnavailable": "内置清单不可用：随包资源文件缺失",
	"settings.quickMessagesLimit": "已达上限 {max} 条（更长的内容建议做成提示词模板）",
	"settings.quickMessagesDragHandle": "拖动排序（也可用上下移按钮）",
	...
	"activity.executionErrorCount": "{count}个错误",
	// 过程组组头文案（契约 §2）：类别文案 + 拼接片段，运行/结束两种时态都要
	...
	"voice.error.configReadFailed": "读取语音转写设置失败，请重试",
	"voice.error.blocked.disabled": "语音输入已关闭，请到常用设置中开启",
	"voice.error.blocked.cloudMissingKey": "云端语音转写缺少 API Key，请到常用设置中填写",
	"voice.error.blocked.cloudMissingEndpoint": "云端语音转写缺少服务地址或模型，请到常用设置中补全",
	"voice.error.blocked.localRuntime": "本地 whisper 未就绪（缺少命令行工具或模型），请到常用设置中安装",
	"voice.error.blocked.unknown": "请先在常用设置中配置语音转写服务",
	...
	"voice.error.empty": "没有识别到语音内容",
	"voice.error.noSpeech": "没有录到声音，请靠近麦克风再试一次",
	"voice.error.staleTarget": "输入内容已发生变化，未插入转写结果",
	"voice.settings.title": "语音输入",
	"voice.settings.description": "选择云端或本地 whisper.cpp 转写。开启后输入框才会出现麦克风按钮；本地模型按需下载，不占用安装包体积。",
	"voice.settings.enabled": "启用语音输入",
	"voice.settings.enabledDescription": "关闭时隐藏输入框的麦克风按钮。",
	"voice.settings.engine": "转写引擎",
	"voice.settings.engineCloud": "云端（在线转写服务）",
	"voice.settings.engineLocal": "本地（whisper.cpp，离线）",
	"voice.settings.device": "录音设备",
	"voice.settings.deviceDescription": "选择麦克风；授权后设备名称才会显示。",
	"voice.settings.deviceDefault": "系统默认",
	"voice.settings.runtime": "本地运行时（whisper-cli）",
	"voice.settings.runtimeReady": "已就绪，可直接转写。",
	"voice.settings.runtimeMissing": "尚未下载，本地转写不可用。",
	"voice.settings.runtimeSourceAuto": "已自动下载",
	...
	"voice.settings.modelDescription": "模型越大越准、越慢、越占空间（小于 181MB 的档位已下架）。选择后可在此下载。",
	"voice.settings.modelFile": "模型文件 · {model}",
	"voice.settings.modelInstalled": "已下载",
	"voice.settings.modelDownload": "下载模型",
	"voice.settings.modelDelete": "删除模型",
	"voice.settings.cliPath": "whisper-cli 路径",
	"voice.settings.cliPathDescription": "可选：手动指定 whisper-cli 可执行文件（自动下载不可用时使用）。",
	"voice.settings.cliPathPlaceholder": "例如 …/whisper-cli.exe 或 /usr/local/bin/whisper-cli",
	"voice.settings.phase.verifying": "校验中",
	"voice.settings.phase.installing": "安装中",
	"voice.settings.phase.done": "完成",
	"voice.settings.phase.error": "失败",
	"voice.settings.installCancel": "取消下载",
	"voice.settings.modelPartial": "已下载 {size}，再次点击将从断点继续",
	"voice.settings.autoSaveHint": "改动会自动保存，无需手动提交",
	"voice.settings.apiKeyAutoSaveHint": "密钥在离开输入框时保存",
	"voice.settings.error.installFailed": "本地语音组件下载安装失败",
	"voice.settings.error.cancelled": "已取消下载",
	"voice.settings.error.alreadyInstalling": "已有一个下载任务在进行中，请先等它结束或取消",
	"voice.error.engineUnavailable": "本地语音引擎尚未就绪，请先在设置中下载运行时与模型",
	...
	"voice.settings.cloudProvider": "云服务商",
	"voice.settings.cloudProviderDescription": "OpenAI 兼容接口走 /audio/transcriptions；豆包语音走「大模型录音文件识别极速版」，需先在 console.volcengine.com/speech 的「开通管理」里开通该模型（新应用赠 20 小时额度）。",
	"voice.settings.cloudProviderOpenai": "OpenAI 兼容",
	"voice.settings.cloudProviderVolc": "火山引擎 · 豆包语音",
	"voice.settings.volcAppId": "App ID",
	"voice.settings.volcAppIdDescription": "控制台「应用管理」里的 App ID（旧版应用）；新版控制台发放的是单个 API Key，直接填在这一格并把下面的 Access Token 留空。",
	"voice.settings.volcAccessToken": "Access Token",
	"voice.settings.volcAccessTokenDescription": "旧版控制台与 App ID 成对必填，只填一个必然鉴权失败；新版控制台留空即可。",
	"voice.settings.cloudResourceId": "资源 ID",
	"voice.settings.cloudResourceIdDescription": "决定服务端按哪套协议解析请求，因此只列出客户端已实现的能力；换服务商协议请等应用支持。",
	"voice.settings.cloudResourceIdTurbo": "大模型录音文件识别 · 极速版",
	"voice.settings.test": "检测连通性",
	"voice.settings.test.ok": "链路正常：服务已受理探测音频，可以开始录音",
	"voice.settings.test.unexpected": "检测请求未能完成，请重试",
	"voice.settings.test.volcHint": "若服务商是豆包语音，请核对控制台是否已开通该资源、额度是否用尽（可凭此码提工单）。",
	"voice.error.blocked.cloudMissingVolcAppId": "豆包语音缺少 App ID，请到常用设置中填写",
	"voice.settings.model": "模型",
	"voice.settings.language": "语言",
	"voice.settings.languageDescription": "可选的 ISO-639-1 语言代码，留空则自动检测；豆包语音会自动映射为 zh-CN 这类区域格式。",
