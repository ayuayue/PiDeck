...
项目位置的并列关系、WSL 全局模式退出路径和旧数据迁移门禁另见 [项目位置架构 ADR（Proposed）](project-location-architecture.md)；该提案待技术评审，不表示三位置并存已经实现，也不放宽 §12 的阶段门禁。
...
> 当前 spike 实测（Windows → Linux 的初步证据，尚非 Phase 0 全部通过）：Windows OpenSSH 9.5p2 → Linux x86_64，Node 24.11.0，Pi 0.85.1。独立核对 fingerprint 后，临时 `known_hosts` pin 下 `get_state`、`agent_start` 后活动中 `abort`、prompt RPC 结算、stdout JSONL 纯净、stdin EOF 退出及断线后精确 PID 回收均通过。独立完整回答测试中，`set_model → get_state` 确认 `jiyuan/deepseek-flash`，不取消的短 prompt 返回成功 `agent_end` 且文本符合预期。非交互 SSH 需为 nvm 下 Pi 临时前置同目录 Node；未改远端账户设置。
>
...
>
> 仍待验证：指纹行为目前只在 Windows OpenSSH 客户端实测；macOS/Linux **客户端**的 `SshHostVerifier` normalized fingerprint 矩阵，以及完整环境变量传播矩阵未覆盖。可先在 Phase 2 开发不联网的主机配置/SSH argv 契约、连接状态机和 helper fixture，且可继续 Windows → Linux 的受控验证；未通过已认证 fingerprint 与环境变量门禁的平台不得激活 profile、启动 helper/Agent/terminal 或暴露远端入口。macOS/Linux 客户端分别在启用连接前补齐验证；Phase 2 不因其尚无实机而停止开发，但不得宣称全平台 Phase 0 已完成。§14.2 的 Windows/macOS 人工 smoke 仍是稳定版发布门禁；若要缩减支持平台，须另行明确变更发布范围，不能将未验证平台当成已支持。远端 Linux 主机与运行 PiDeck 的 Linux 客户端是两项不同的验证。
